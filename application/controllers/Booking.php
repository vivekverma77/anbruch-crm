...
class Booking extends Base
{
			...
			if(empty($booking))
			{
					...
					if($bookingId)
					{
						...
						$notifyData = array(
							...
							"description" =>  $this->getPost("booking_title").' on '.date('m/d/Y',strtotime($this->getPost("selected_date"))).' at '.date('H:i',strtotime($this->getPost("selected_time"))),
	...
	public function bookupdate()
	{
        ...
        $booking_date = date('Y-m-d H:i:s',strtotime($_POST['start_date'].' '.$_POST['start_time']));

        $booking_id=$_POST['booking_id'];
        $created_for=$_POST['created_for'];
         
        $booking = $this->BookingModel->check_booking_availability($booking_date,$booking_id,$created_for,true);
		...
		if(isset($jobData['dragEvent']))
		{
			...
			$email = $jobData['email'];
			$current_module = $jobData['current_module'];
			...
			if($status)
			{
				...
				$data = array(
					...
					'booking_time' => date('H:i',strtotime($res[0]['booking_date'])),
					'end_date' => date('m/d/Y',strtotime($res[0]['end_date'])),
					'end_time' => date('H:i',strtotime($res[0]['end_date'])),
				...
				$this->ModuleModel->saveEmail('',$fromEmail,$toEmail,$ccEmail="",$bccEmail="",$title,$emailTemplate);
				    ...
				    $logid = $this->LogsModel->addLogs('Booking', 'Booking is Updated' ,$record_id, $current_module,$_SESSION['user_id'],'Develop'); 

					$result = array("message" => "Booking Updated successfully",
									"success" => true);

	// for exists guest email
			  $res = $this->App_model->getData('anb_crm_bookings', array('*'), array('id' => $booking_id));
	          $record_id = $res[0]['record_id'];
	          $mid = 31;
	          $company_name =$this->db->get_where('anb_crm_record_meta_value',['record_id'=>$record_id,'record_meta_id'=>$mid]);
	          $res1=$company_name->result();
	        // echo "<pre>";print_r($res);die("test");
	          $booking_title = $res[0]['booking_title'];
						...
						foreach ($ret as $key => $value) {
						  ...
						  if(filter_var($email, FILTER_VALIDATE_EMAIL)){
						  	$this->email->clear(TRUE);
								...
								$invitationData = $data;
								$invitationData['acceptUrl'] = base_url("user/webBookingInvitation/?action=View&bid=$booking_id&email=$email&status=1&invitationId=$flag");

								$invitationData['declineUrl'] = base_url("user/webBookingInvitation/?action=View&bid=$booking_id&&email=$email&status=3&invitationId=$flag");

								$emailTemplate = $this->load->view('emails/openercloserbooking',$invitationData,true);
							
								$subject = 'Booking Invitation | '.date('F d', strtotime($jobData["start_date"])).' @'.date("h:i a",strtotime($jobData["start_time"])).' EST';

								$fromEmail = COMPANY_NOREPLY_EMAIL;
								$fromName = COMPANY_NAME;
								$toEmail = trim($email);

								$this->ModuleModel->saveEmail($this->getUserId(),$fromEmail,$toEmail,$ccEmail="",$bccEmail="",$subject,$emailTemplate);
								...
								$this->email->send();
  //echo "<pre>";print_r($data);   var_dump($emailTemplate);die;
	...
	//for opener email	
					$opener =$this->db->get_where('anb_crm_record_meta_value',['record_id'=>$record_id,'record_meta_id'=>22]);
	                $op_res=$opener->result();
					$op_email = $this->UserModel->loadUser($op_res[0]->value);
					$opener_email = trim($op_email['email']);
					//var_dump($opener_email);

						  if(filter_var($opener_email, FILTER_VALIDATE_EMAIL)){
						  	$this->email->clear(TRUE);
							
								$invitationData = $data;
								$invitationData['acceptUrl'] = base_url("user/webBookingInvitation/?action=View&bid=$booking_id&email=$opener_email&status=1&invitationId=$flag");

								$invitationData['declineUrl'] = base_url("user/webBookingInvitation/?action=View&bid=$booking_id&&email=$opener_email&status=3&invitationId=$flag");
  ...
  //echo "<pre>";print_r($data);  
               // var_dump($opener_email);  var_dump($emailTemplate);var_dump("ok");

						}
    //for opener email end		
			

	//for closer email	
					$closer =$this->db->get_where('anb_crm_record_meta_value',['record_id'=>$record_id,'record_meta_id'=>21]);
	                $cl_res=$closer->result();
					$cl_email = $this->UserModel->loadUser($cl_res[0]->value);
					$closer_email = trim($cl_email['email']);

						  if(filter_var($closer_email, FILTER_VALIDATE_EMAIL)){
						  	$this->email->clear(TRUE);
							
								$invitationData = $data;
								$invitationData['acceptUrl'] = base_url("user/webBookingInvitation/?action=View&bid=$booking_id&email=$closer_email&status=1&invitationId=$flag");

								$invitationData['declineUrl'] = base_url("user/webBookingInvitation/?action=View&bid=$booking_id&&email=$closer_email&status=3&invitationId=$flag");
								...
								$this->email->send();
  //echo "<pre>";print_r($data);   
								// var_dump($closer_email);
								//  die("test");

						}
	//for closer email end	
		...
		if($ct >= $st)
		{
			$result['message'] = 'Booking should not updated in past';
   			$result['success'] = false;
   			$this->session->set_flashdata('error',"Booking should not updated in past");
   			echo json_encode($result); die(); 	
		}
		if($st >= $et)
		{
		...
		if(!empty($jobData['notification_type']) && !empty($jobData['notification_interval']) && !empty($jobData['notification_via']) && $jobData['notification_interval']!=null){
			...
			if($recordUpdateSuccessful === true)
			{
				...
				$data = array(
					...
					'booking_time' => date('H:i',strtotime($this->getPost("start_time"))),
					'end_date' => date('m/d/Y',strtotime($this->getPost("end_date"))),
					'end_time' => date('H:i',strtotime($this->getPost("end_time"))),
					'booking_title' => $booking_title,
	        ...
	        // echo "<pre>";print_r($res);die("test");
	          ...
	          $data = array(
				 ...
				 if(!empty($jobData['invite']))
				{
					...
					foreach ($jobData['invite'] as $key => $email) {
...
// for exists guest email
				 if(!empty($jobData['invite_exist']))
				{
					
					foreach ($jobData['invite_exist'] as $key => $email) {
					  ...
					  if(filter_var($email, FILTER_VALIDATE_EMAIL)){
					  	$this->email->clear(TRUE);
							...
							//$this->email->attach($pdfPath);
							$this->email->send();
  // echo "<pre>";var_dump($toEmail);var_dump($emailTemplate); die;
  ...
  //echo "<pre>";print_r($data);   
	...
	//for closer email end			
			...
			if($google_event_id!='')
			{
				...
				$description .='<br/><br/><b>Booking time</b>: '. date( 'H:i',strtotime($dateArr[1]) );
	...
	public function remove_event_guest()
	{
		$event_guest_id = $_POST['id'];
		$this->db->where('id',$event_guest_id);
		$this->db->delete('calendar_invitations');
	...
	public function index()
	{
		...
		{
			$user_array = [];
		  if($this->isSuperAdmin() || $_SESSION['role_id']==7){
		  	$this->data['show_booking_all']='all';
		  	$user_array[]='all';
		  }else{
		...
		$this->data["optional_users_list"] = $this->ModuleModel->loadOptionalUsersList(1, $loggedUserId);
		 //echo '<pre>';print_r($this->data["own"]);die("tsts");
		...
		//if($this->isSuperAdmin() || $own_data['role_id'] == 7)
		if($user_array[0] == "all")
		{
			//$resultArray = $this->App_model->getData('anb_crm_bookings', '*, id as booking_id', array('web_booking' => 1));
			
			$bookings_result = array_merge_recursive($this->data["bookings"], $this->data["guest_booking"]);

			//echo '<pre>'; print_r($xyz); die("ok");
			//$bookings_result = array_merge_recursive($xyz, $resultArray);		 
		}
		else
		{
			$bookings_result = array_merge_recursive($this->data["bookings"], $this->data["guest_booking"]);

			//echo '<pre>'; print_r($bookings_result); die("ok");
		...
		{
			$agenda = array();
			$agenda['module_id'] = !empty($booking['booking_module_id']) ? $booking['booking_module_id'] : 1;
			...
			$agenda['praposed_date'] = date('m/d/Y H:i',strtotime($booking['praposed_date']));
			$agenda['praposed_end_time'] = date('H:i',strtotime($booking['praposed_end_time']));
			$agenda['company']=$booking['value'];
			...
			// if($booking['replace_book'] == 1){
			//   $agenda['class'] =  'replace_book';
		 //    }
			//date_default_timezone_set("Asia/Kolkata");

			$date1 = date('Y-m-d H:i');
			//$date1 = date('Y-m-d H:i:s',strtotime('+5 hour +30 minutes',strtotime($date1)));
			$date2 = date('Y-m-d H:i',strtotime($date.' '.$time));
			...
			if($booking['status']==0)
			{
				...
				{	if(strtotime($date2) < strtotime($date1))
					{
						$agenda['backgroundColor'] =  '#fc3b8c';    // faded blue
					}
					else
					{
						$agenda['backgroundColor'] =  $booking['color']!="" ? $booking['color'] : '#fc3b8c';    // blue
					}
					$st2 = '<span style="display:block;font-size: 12px;top: -4px;position: relative;color:#fc3b8c;"><i class="fa fa-exclamation-circle"></i> Awaiting for confirmation</span>';
			...
			{
				$agenda['backgroundColor'] = '#40409c';
				$st2 = '<span style="display:block;font-size: 12px;top: -4px;position: relative;color:#40409c;"><i class="fa fa-exclamation-circle"></i> Web Booking</span>';
				$st = 'Web Booking';
			}
			else if($booking['status']==1)
			{
			...
			else if($booking['status']==2 && $booking['praposed_date'] != 0)
			{
				...
				$st2 = '<span style="display: block;font-size: 12px;top: -4px;position: relative;color:#ffbc00;"><i class="fa fa-times-circle"></i> Cancelled</span><div style="font-size: 12px;">Suggested Time by Client: </span><span style="color:#ffbc00;">'.date("m/d/Y H:i",strtotime($booking['praposed_date'])).'</div> </div> </div> <div class="clearfix" style="border-top:2px dashed #ddd; padding-top:15px;"><span class="rebooking_availability"><a href="javascript:" class="check_rebooking btn-link" style="font-size:12px;">Check Availability</a></span>  <span class="fade re_booking_other pull-right" style="display:none"><span class="hover-title">Rebook Other</span> </span>     <span class="rebooking-status"> </span> <span class="fade re_booking pull-right" style="display:none"><span class="hover-title">Rebook Client</span> </span></div> <div> <div>';
				$st = 'Cancelled';
			...
			if($booking['all_day']==1)
			{
				$endtime = date('H:i',strtotime('+23 hour +59 minutes',strtotime($time)));
				$all_day = '(All Day)';
			}
			else
			{
				$endtime = date('H:i',strtotime($booking['end_date']));
				$all_day = '';
			}
			...
			/**/
			$description = '';
			// echo "<pre>"; print_r($booking); die("Ok");
			$description .= '<input type="hidden" id="s_book_id_record" value="'.$booking["booking_id"].'">'; 
			$description .= '<input type="hidden" id="s_book_created_id" value="'.$booking["created_for"].'">'; 
			...
			$description .= '<input type="hidden" id="stime" value="'.date("H:i",strtotime($booking['praposed_date'])).'">';
			

			if($booking['all_day'] == 1){
				$description .= '<div class="eventView-row col-xs-12 "><div class="col-xs-2 event-icon"><span class="event-color"></span></div><div class="col-xs-11 event-div"><div class="jobName col-xs-12 edit-title">'.$booking['booking_title'].'</div><div class="startDay col-xs-5" style="background-color: #f3ebeb;padding-left:5px;">'.$day.', '.$date.'</div><div class="endDate col-xs-4" style="background-color: #f3ebeb;padding-left:10px;">'.date("H:i",strtotime($booking["end_date"])).'</div></div></div>';
				
			}else{
				$description .= '<div class="eventView-row col-xs-12 "><div class="col-xs-2 event-icon"><span class="event-color"></span></div><div class="col-xs-11 event-div"><div class="jobName col-xs-12 edit-title">'.$booking['booking_title'].'</div><div class="startDay col-xs-5" style="background-color: #f3ebeb;padding-left:5px;">'.$day.', '.$date.' </div><div class="endDate col-xs-4" style="background-color: #f3ebeb;padding-left:10px;">'.$time.' - '.date("H:i",strtotime($booking["end_date"])).'</div></div></div>';
			...
			if($booking['status']==0  && $booking['web_booking'] == 1)
					{
					  $wb = '<span style="display:block;font-size: 12px;margin-top: 5px;;color:#40409c;float:right"><i class="fa fa-exclamation-circle"></i> Web Booking</span>';
					}else{
						$wb = "";
					}	
			$description .= '<div class="eventView-row col-xs-12"> <div class="clearfix"><div class="col-xs-2 address-icon"><i class="fas fa-user"></i></div><div class="field-col col-xs-10 event-address" style="width:85%"><span class="semail">'.$booking['email'].'</span>'.$wb.' '.$st2.'</div></div></div>';
			...
			if(!empty($agenda['notification'])){
				...
				else if($agenda['notification'][0]['notification_type']==2){
					$description .='<div class="eventView-row col-xs-12" ><div class="col-xs-2 address-icon"><i class="fa fa-bell"></i></div> <div class="col-xs-10 event-address">Before ' . $agenda['notification'][0]['notification_interval'].' Day Via '.$via.'</div></div>';
				}
				else{
					$description .='<div class="eventView-row col-xs-12" ><div class="col-xs-2 address-icon"><i class="fa fa-bell"></i></div> <div class="col-xs-10 event-address">Before ' . $agenda['notification'][0]['notification_interval'].' minutes Via '.$via.'</div></div>';
				}
			}
			...
			$description .= '<div class="eventView-row col-xs-12" ><div class="col-xs-2 address-icon"><i class="fas fa-briefcase" ></i></div> <div class="company_name col-xs-10 event-address">'.$booking['value'].'</div></div>';
	...
	public function bookNow()
	{
		...
		if(!empty($jobData['notification_type']) && !empty($jobData['notification_interval']) && !empty($jobData['notification_via'])){
        	...
        	$booking = $this->BookingModel->check_booking_availability($booking_date,$booking_id,$created_for,false);
			//print_r($booking);
			if(empty($booking))
			{
				$st = $booking_date;
				...
				if($st < date('Y-m-d H:i')){

					$result['message'] = 'Booking Could not updated in past';
	       			$result['success'] = false;
	       			$this->session->set_flashdata('error',"Booking Could not updated in past");
	       			echo json_encode($result); die(); 
				}
				...
				if($bookingId)
				{
					...
					$logid = $this->LogsModel->addLogs('Booking', 'Booking is Done' ,$record_id, $current_module,$_SESSION['user_id'],'Develop'); 
					$send_email = false;
					...
					if($leadUpdate){
				     ...
				     //Opener email send
						...
						$emailTemplate = $this->load->view('emails/openercloserbooking',$assignedData,true);
					...
					if($current_module == 'Clients')
					{
						...
						$assignedData = array(
							...
							'lead_name' => isset($client_name) ? $client_name : '',
				  ...
				  // Closer email send
					...
					$notifyData = array(
						...
						"description" =>  $this->getPost("booking_title").' on '.date('m/d/Y',strtotime($this->getPost("start_date"))).' at '.date('H:i',strtotime($this->getPost("start_time"))),
						"read" => 0, //0=new booking
						"created_time" =>date('Y-m-d H:i:s'),
						"modified_time" =>date('Y-m-d H:i:s'),
					);
	...
	//zoomapi start
		$zoom_active = $this->getPost('conference');
		if($zoom_active == 2)
		{
			$topic = $this->getPost("booking_title");
			$time = $this->getPost("start_date")." ".$this->getPost("start_time");
			$starttime = date("Y-m-d\TH:i:s",strtotime($time));
			$create_id = $this->create_meeting($bookingId,$topic,$starttime);

			$this->db->select('*');
		  	$this->db->from('zoom_meeting');
		  	$this->db->where('Booking_id',$bookingId);
		  	$data =  $this->db->get();
		  	$q_data= $data->result_array();
			$Zoom_link = $q_data[0]['link'];
					...
					foreach ($jobData['invite'] as $key => $email) {
					  ...
					  if(filter_var($email, FILTER_VALIDATE_EMAIL)){
						...
						if($flag)
						{
							...
							$invitationData['acceptUrl'] = base_url("user/webBookingInvitation/?action=View&bid=$bookingId&email=$email&status=1&invitationId=$flag");

							$invitationData['declineUrl'] = base_url("user/webBookingInvitation/?action=View&bid=$bookingId&&email=$email&status=3&invitationId=$flag");
	...
	function cancelEventToGoogleCalendar()
	{
		...
		if($booking_data['google_event_id']!='')
		{
			...
			$date = $dateArr[0].'T'.$dateArr[1];
			$enddate = $dateArr[0].'T'.  date('H:i',strtotime($dateArr[1])+ 60*30 );

			$description ='<br/><br/>Status: Booking Cancelled!';
			$description .='<br/><br/>Booking date: '. date( 'm/d/Y',strtotime($dateArr[0]) );
			$description .='<br/><br/>Booking time: '. date( 'H:i',strtotime($dateArr[1]) );
			...
			if(!empty($bookings))
			{
				...
				if($res)
				{
					...
					$notifyData = array(
								...
								"description" =>  $bookings[0]["booking_title"].' on '.date('m/d/Y').' at '.date('H:i'),
								"read" => 0, //0=new booking
								"created_time" =>date('Y-m-d H:i:s'),
								"modified_time" =>date('Y-m-d H:i:s'),
							);
	...
	public function cancelBooking()
	{
		...
		if ( ($requestMethod = strtolower($this->input->method())) == 'post')
		{
				...
				$booking = $this->BookingModel->loadBookingById($bookingId);
				 ...
				 //	echo "<pre>"; print_r($res); die;
						...
						$notifyData = array(
								...
								"description" =>  $res[0]["booking_title"].' on '.date('m/d/Y').' at '.date('H:i'),
           ...
           // echo "<pre>"; print_r($notifyData); die;
					...
					$data = array(
						'booking_date' => date('m/d/Y',strtotime($bookings[0]["booking_date"])),
						'booking_time' => date('H:i',strtotime($bookings[0]["booking_date"])),
						'end_date' => date('m/d/Y',strtotime($booking[0]["end_date"])),
						'end_time' => date('H:i',strtotime($booking[0]["end_date"])),
						'name' => $bookings[0]["name"],
						'email' => $bookings[0]["email"],
						'current_module' =>!empty($this->getPost("current_module")) ? $this->getPost("current_module") : '',
						'current_record_id' => !empty($this->getPost("current_record_id")) ? $this->getPost("current_record_id"): '',
  ...
  public function get_booking(){
	...
	public function followlead(){
				...
				$leadUpdate = $this->db->where('record_id', $record_id)
					...
					if($leadUpdate){
						$this->LogsModel->addLogs('Follow Lead', 'Lead is followed', $record_id, 'Leads', $_SESSION['user_id'],'Prospect');
	...
	public function unfollowlead(){//unfollow lead
				...
				if($updateLead){
					$this->LogsModel->addLogs('unfollow Lead', 'Lead is unfollowed', $record_id, 'Leads', $_SESSION['user_id'],'Prospect');
					$updatetime = $this->db->set('modified_time', $modified_time)
																 ->where('id', $record_id)
																 ->update('anb_crm_record');}
	...
	public function massUnfollowlead(){//unfollow lead
				$record = array('value' => 'Cold',); //set value to null for the unfollow
				$record_id = $this->input->post("record_id");
				...
				for ($i=0; $i <count($array); $i++) { 
					...
					$modified_time = time();
						...
						$updateLead = $this->db->where('record_id', $array[$i])
							 	     							 ->where('record_meta_id', 14)
							       			 				 	->update('anb_crm_record_meta_value', $record);
					if($updateLead){
						$this->LogsModel->addLogs('unfollow Lead', 'Lead is unfollowed', $array[$i], 'Leads', $_SESSION['user_id'],'Prospect');
						$updatetime = $this->db->set('modified_time', $modified_time)
																	 ->where('id', $array[$i])
																	 ->update('anb_crm_record');}
	...
	public function followClientContract(){
			...
			if($mid==223){
				...
				$res=$this->db->get_where('anb_crm_record_meta_value',['record_id'=>$record_id,'record_meta_id'=>$mid]);
				$res=$res->result();

				if(empty($res)){
					
					$insert=$this->db->insert('anb_crm_record_meta_value',
						              ['record_id'=>$record_id,
						               'record_meta_id'=>$mid,
						               'value'=>'Hot']);
					...
					if($insert){
						...
						$this->LogsModel->addLogs("Follow $module", "$module is followed", $record_id, "$module", $_SESSION['user_id'],'Prospect');
						$updatetime = $this->db->set('modified_time', $modified_time)
															   ->where('id', $record_id)
															   ->update('anb_crm_record');
					}

					$this->session->set_flashdata('success',"Contracts followed successfully");
					die();
			
				}	
			}
			$Update = $this->db->where('record_id', $record_id)
								   ->where('record_meta_id', $mid)
							  	   ->update("anb_crm_record_meta_value", $recordData);
		    if($Update){
				$this->LogsModel->addLogs("Follow $module", "$module is followed", $record_id, "$module", $_SESSION['user_id'],'Prospect');
				$updatetime = $this->db->set('modified_time', $modified_time)
			...
			$this->session->set_flashdata('success',"$module followed successfully");
	}

	public function unfollowClientContract(){
		...
		$modified_time = time();
			...
			$update = $this->db->where('record_id', $record_id)
				 	     							 ->where('record_meta_id', $mid)
				       			 				 	->update('anb_crm_record_meta_value', $record);
		if($update){
			$this->LogsModel->addLogs("unfollow $module", "$module is unfollowed", $record_id, "$module", $_SESSION['user_id'],'Prospect');
			$updatetime = $this->db->set('modified_time', $modified_time)
														 ->where('id', $record_id)
														 ->update('anb_crm_record');}
		$this->session->set_flashdata('success',"$module Unfollowed successfully");

	}

	public function massClientContractUnfollow(){
		...
		$modified_time = time();
			...
			$update = $this->db->where('record_id', $array[$i])
				 	     							 ->where('record_meta_id', $mid)
				       			 				 	->update('anb_crm_record_meta_value', $record);
	     			       			 				 	
		if($update){
			$this->LogsModel->addLogs("unfollow $module", "$module is unfollowed", $array[$i], "$module", $_SESSION['user_id'],'Prospect');
			$updatetime = $this->db->set('modified_time', $modified_time)
														 ->where('id', $array[$i])
														 ->update('anb_crm_record');}
   ...
   public function first_checkAvailability()
   {
   		$record_id = $this->input->post('record_id');
   		$email = $this->input->post('email');
   		$start_date = $this->input->post("start_date");
		$start_time = $this->input->post("start_time");
		$booking_date = date('Y-m-d H:i',strtotime($start_date.' '.$start_time));

   		//var_dump($record_id); var_dump($email);
   ...
   public function checkAvailability()
	{
		...
		if ( ($requestMethod = strtolower($this->input->method())) == 'post')
		{
			...
			if(isset($_POST["created_for"]) && !empty($_POST["created_for"]))
			{
			  $user = $this->App_model->getData('anb_crm_users_personal_info',array('*'),array('user_id' => $_POST["created_for"]));
			}
			else{
				$user = "";
			}
			...
			$result['status'] = "Booking Not Available for ".$user[0]['first_name']." ".$user[0]['last_name'];
	...
	public function webBooking($rowno = 0)
	{
		$sel= $this->data["sel"] = ($this->getPost("sel") != '') ? $this->getPost("sel") : "";
		$rowperpage = isset($sel) && !empty($sel) ? $sel : 10;
		if($rowno != 0){
	      $rowno = ($rowno-1) * $rowperpage;
	    }
	    $allcount = $this->App_model->getRowCount('anb_crm_web_booking', '*');
		$order_by = ($this->getPost("order_by") != '') ? $this->getPost("order_by") : 'created_at';
	    $order = ($this->getPost("order") != '') ? $this->getPost("order") : 'desc';
	    
	    $users_record = $this->BookingModel->getWebBookingData($rowno,$rowperpage,$order,$order_by);

		// Pagination Configuration
	    $config['base_url'] = base_url().'Booking/webBooking';
	    ...
	    $this->data['users_list']=$this->ModuleModel->loadUsersList();
		$this->viewLoad('booking/webBooking');
	}

	public function booking_details(){
		$id = $this->input->post('id');
		$data = $this->BookingModel->booking_details($id);
		$notification= $this->App_model->getData('event_notification','*', 'event_id = '.$id);

			$day = date("l", strtotime($data[0]["booking_date"]));
			$date = date("m/d/Y", strtotime($data[0]["booking_date"]));
			$time = date("H:i", strtotime($data[0]["booking_date"]));
			$endtime = date("H:i", strtotime($data[0]["end_date"]));

		$data['booking_date'] = $day.", ".$date." ". $time ." - ".$endtime; 
		$data['end_time'] = $endtime; 

		  if($data[0]['status'] == 1){
		        $data['status'] = '<span style="color:#33b679;font-size:13px;"><i class="fa fa-exclamation-circle"></i> <span style="font-size:14px;">Confirmed</span></span>';
		        $data['event_color'] = "#33b679";
		    }else if($data[0]['status'] == 2 && $data[0]['praposed_date'] == '0000-00-00 00:00:00'){
		        $data['status'] = '<span style="color:#c9302c;font-size:13px;"><i class="fa fa-exclamation-circle"></i> <span style="font-size:14px;">Cancelled</span></span>';
		        $data['event_color'] = "#c9302c";

		    }else if($data[0]['status'] == 2 && $data[0]['praposed_date'] != '0000-00-00 00:00:00'){
		        $data['status'] = '<span style="color:#ffbc00;font-size:13px;"><i class="fa fa-exclamation-circle"></i> <span style="font-size:14px;" title="Suggested Time: '.$data[0]['praposed_date'].'">Time Suggested</span></span>';
		        $data['event_color'] = "#ffbc00";

		    }else{
		        $data['status'] = '<span style="color:#fc3b8c;font-size:13px;"><i class="fa fa-exclamation-circle"></i> <span style="font-size:14px;">Awaiting for confirmation</span></span>';
		        $data['event_color'] = "#fc3b8c";
		    }

		    if(!empty($notification)){
				$via = $notification[0]['notification_via'] == 1 ? 'Email':'Notification';

				if($notification[0]['notification_type']==1){
					$data['notification'] ='<div class="col-xs-2 address-icon"><i class="fa fa-bell"></i></div>  <div class="col-xs-10 event-address">Before ' . $notification[0]['notification_interval'].' Hour Via '.$via.'</div>';
				}
				else if($notification[0]['notification_type']==2){
					$data['notification'] ='<div class="col-xs-2 address-icon"><i class="fa fa-bell"></i></div> <div class="col-xs-10 event-address">Before ' . $notification[0]['notification_interval'].' Day Via '.$via.'</div>';
				}
				else{
					$data['notification'] ='<div class="col-xs-2 address-icon"><i class="fa fa-bell"></i></div> <div class="col-xs-10 event-address">Before ' . $notification[0]['notification_interval'].' minutes Via '.$via.'</div>';
				}
			}

	//var_dump($date); var_dump($time);	print_r($day);die;
		if($data){echo json_encode($data); die;}else{echo json_encode("error");die;}
	}
...
// Zoom functionality start	
		...
		public function configg(){
			if (!defined('CLIENT_ID')) { define('CLIENT_ID', 'LeVVOrQTICJ0iMuBsBBVQ'); }
			if (!defined('CLIENT_SECRET')) { define('CLIENT_SECRET', 'XalUkp7MurHmv4Vw1hm2nbp6yfeBtbKf'); }
			 if (!defined('REDIRECT_URI')) {  define('REDIRECT_URI', 'https://softgetix.com/projects/crm-anbruch/booking/callback'); }
		}

		public function create_meeting($bookingId,$topic,$starttime) {
	  			$this->configg();
		    ...
		    try {
		        ...
		        $data=array("topic_name"=>$topic,"Booking_id"=>$bookingId,"start_time"=>$starttime,"duration"=>30,"password"=>"123456","meeting_id"=>$id,"link"=>$url_main);
		        $sql=$this->db->insert('zoom_meeting',$data);
		        ...
		        if( 401 == $e->getCode() ) {
		           ...
		           $this->create_meeting($bookingId,$topic,$starttime);
		        } else {
		            echo $e->getMessage();
		        }
		    }
