    ...
    $this->data['search_filters'] = $arr_search_filters;
		...
		if(isset($_GET['alphabet']))
		{
			$arr_temp = $this->ModuleModel->loadRecordList_beta($this->data["recordStatus"], $this->data["own"], $arr_search_filters, $this->data["page"], $limit_per_page, $this->data['sort_column'], $this->data['sort_order'], $this->data['keyword'],$this->data["current_module"], $from_date,  $this->getPost('id'),$_GET['alphabet']);
			 //echo $this->db->last_query();die("one");
			$this->data['alphabet'] = $_GET['alphabet'];
		}
		else if(!empty($_GET['fl_id']))
		{
			$arr_temp = $this->ModuleModel->loadRecordList_beta($this->data["recordStatus"], $this->data["own"], $arr_search_filters, $this->data["page"], $limit_per_page, $this->data['sort_column'], $this->data['sort_order'], $this->data['keyword'],$this->data["current_module"], $from_date,  $this->getPost('id'),'',$_GET['fl_id']);
			$this->data['selected_fav_list'] = $_GET['fl_id'];	
			//echo "<pre>";print_r($this->data['selected_fav_list']);die;
		}
		else
		{
...
//for additional field remove
		if($this->data["current_module"] == "Clients")
		{	
		  foreach ($arr_temp[0] as $key => $record)
			{
					 unset($arr_temp[0][$key]['meta']['Client Category']);
					 unset($arr_temp[0][$key]['meta']['Company Name']);
			        $result = $arr_temp[0];
			}
...
//for additional field remove
		...
		$this->data['favorite_list'] = $this->App_model->getData('anb_crm_favorite_list','*', array('created_by' => $this->session->userdata('user_id')));

		//echo '<pre>';print_r($this->data['favorite_list']);die("test");
		...
		if(($requestMethod = strtolower($this->input->method())) == 'post')
		{
			...
			if($this->data['current_module'] == 'Clients' && $this->data['action'] == 'addContract')
			{
				...
				if($result){
					...
					$this->LogsModel->addLogs('Contract', 'New Contract is added', $message["id"], $this->data["current_module"], $_SESSION['user_id'],'Prospect');
					$this->LogsModel->addLogs('Contract', 'New Contract is added', $owner_id, $this->data["current_module"], $_SESSION['user_id'],'Prospect');
               ...
               if($this->data["current_module"]=='Leads'){
					$this->LogsModel->addLogs('Leads', 'Leads Added',$message['id'],$this->data["current_module"],$this->getUserId(),'Prospect');
               }
			}
			if($this->data["current_module"]=='Leads')
			{
				...
				if ($message["status"] === true && $message["id"]!='')
				{
                    ...
                    redirect('modules/viewRecord/?cm='. $this->data["current_module"] .'&id='.$message["id"] );
			...
			if($this->data["current_module"]=='Clients' && $this->data['action'] == '')
			{
				if ($message["status"] === true && $message["id"]!='')
				{
					...
					$this->data["message"]["success"] = 'Record has been added successfully. <a href="'. base_url() .'modules/viewRecord/?cm='. $this->data["current_module"] .'&id='. $message["id"] .'">View The Record?</a>';
					$this->LogsModel->addLogs('Clients', 'Clients Added',$message['id'],$this->data["current_module"],$this->getUserId(),'Prospect');
			...
			if($this->data["current_module"]=='Contracts')
			{
				...
				if($result){
					$record_data = $this->ModuleModel->loadRecord1(null, $contract_id);
					$this->LogsModel->addLogs('Contract', 'New Contract is added', $message["id"], $this->data["current_module"],$_SESSION['user_id'],'Prospect');
					$this->LogsModel->addLogs('Contract', 'New Contract is added', $_POST['record_id'], $this->data["current_module"],$_SESSION['user_id'],'Prospect');
...
*/
			...
			if($this->data["current_module"]=='Clients' && $this->data['action'] == 'addContract')
			{
				if($message["status"] === true && $message["id"]!='')
				{	
					$this->LogsModel->addLogs('Contract', 'New Contract is added', $this->getPost('clientId'), $this->data["current_module"], $_SESSION['user_id'],'Prospect');
		...
		if(($requestMethod = strtolower($this->input->method())) == 'post')
		{
			...
			if($this->data["current_module"]=='Leads')
			{
				if($message["status"] === true && $this->data["record_id"]!='')
				{
                    ...
                    redirect('modules/viewRecord/?cm='. $this->data["current_module"] .'&id='.  $this->getPost('id'));
	...
	public function viewRecord()
	{
		...
		if ( $this->getPost("ac") == 'del' )
		{
				...
				if ($message["status"] === true){
						...
						$this->session->set_flashdata('success', "Record has been " . $this->data["action"] . "d successfully");
						$this->viewLoad("modules/view");
				} else {
						$this->session->set_flashdata("error", "Record cannot be deleted now. Please try again later.");
						$this->data["message"]["danger"] = 'Error! Record cannot be deleted now. Please try again later.';
				}
			redirect(base_url().'modules/?cm='.$this->getPost('cm').'&id=1');
		}
		...
		else if ( $this->getPost("ac") == 'convert' )
		{
				...
				if($convertIssue)
				{
					$record_id = $this->postGet('id');
					$this->LogsModel->addLogs('Lead Converted to Client', 'A Lead is Converted to Client' ,$convertIssue,$this->getPost("cm"),$this->getUserId(),'Closed');
		...
		if ($loadViewWithData)
		{
			...
			$this->data["primary_contact"] = $this->App_model->getData('anb_crm_record','primary_contact', array('id' => $this->data["current_record_id"]))[0]['primary_contact'];
			...
			$q =  $this->db->query("select * from anb_crm_favorite_list where records LIKE '%".$this->data["current_record_id"]."%'");
			$this->data['favorite_list_records'] = $q->result_array();
			...
			$this->data['proposal']=$this->App_model->getData('proposal_document','*',['record_id'=>$this->data["current_record_id"]]);
...
/*				 	print_r($this->data['qualified']);
	...
	// PERSONAL COUNTRY START		
			$pp_country = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 248));
			$arr = explode(',', $pp_country[0]['option_value']);
			$this->data['country'] = $arr;
	// PERSONAL COUNTRY END
    //Sales Tax Registration START		
			$s_tax = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 17,'module_id'=>1));
			$st = explode(',', $s_tax[0]['option_value']);
			$this->data['salex_tax'] = $st;
	// Sales Tax Registration END	
	//Sales Tax status  START		
			$s_tax_status = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 18,'module_id'=>1));
			$sts = explode(',', $s_tax_status[0]['option_value']);
			$this->data['salex_tax_status'] = $sts;
	// Sales Tax Registration END	
		//Sales Tax status  START		
			$service_type = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 9,'module_id'=>1));
			$service = explode(',', $service_type[0]['option_value']);
			$this->data['service_type'] = $service;
	// Sales Tax Registration END	
	//Sales Tax status  START		
			$lead_Call_Status	 = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 27,'module_id'=>1));
			$call_status = explode(',', $lead_Call_Status[0]['option_value']);
			$this->data['lead_call_status'] = $call_status;
	// Sales Tax Registration END	
    //Lead category START		
			$lead_Category	 = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 14,'module_id'=>1));
			$category = explode(',', $lead_Category[0]['option_value']);
			$this->data['lead_category'] = $category;
	//Lead category END	
	//Lead status START		
			$lead_status	 = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 6,'module_id'=>1));
			$l_status = explode(',', $lead_status[0]['option_value']);
			$this->data['lead_status'] = $l_status;
	//Lead status END
	//Closer START
			$closerjoin = array(
	    			array(
			...
			$closer	= $this->App_model->getData('anb_crm_users a', array('b.first_name', 'b.user_id', 'a.role_id','a.status', 'b.last_name'), array('role_id'=>4,'status'=>1), $closerjoin);
			$this->data['lead_closer'] = $closer;
	//Closer END
	//Opner START
			$opnerjoin = array(
	    			array(
			...
			$opner	= $this->App_model->getData('anb_crm_users a', array('b.first_name', 'b.user_id', 'a.role_id','a.status', 'b.last_name'), array('role_id'=>3,'status'=>1), $opnerjoin);
			$this->data['lead_opner'] = $opner;
	//Opner END	
	//Lead source START		
			$lead_source	 = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 5,'module_id'=>1));
			$l_source = explode(',', $lead_source[0]['option_value']);
			$this->data['lead_source'] = $l_source;
	//Lead source END	
	//Lead source START		
			$id_source	 = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 11,'module_id'=>1));
			$lead_id_source = explode(',', $id_source[0]['option_value']);
			$this->data['lead_id_source'] = $lead_id_source;
	//Lead source END	
	//Client status START		
			$client_status	 = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 87,'module_id'=>2));
			$c_status = explode(',', $client_status[0]['option_value']);
			$this->data['client_status'] = $c_status;
	//Client status END		
	//Contract stage START		
			$contract_stage	 = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 162,'module_id'=>3));
			$stage = explode(',', $contract_stage[0]['option_value']);
			$this->data['contract_stage'] = $stage;
	//contract stage END	
	// contract service type start
			$contract_service_type = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 163,'module_id'=>3));
			$con_service = explode(',', $contract_service_type[0]['option_value']);
			$this->data['contract_service_type'] = $con_service;
	//contract service type end	
	// client service type start
		    $client_service_type = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 90,'module_id'=>2));
			$this->data['client_service'] = explode(',', $client_service_type[0]['option_value']);
	//client service type end
	//Technical Consultant:	 START
			$join = array(
	    			array(
	    				'table' => 'anb_crm_users_personal_info b',
	    				'on' => 'a.id = b.user_id',
			...
			$this->data['tech_const'] = $this->App_model->getData('anb_crm_users a', array('b.first_name', 'b.user_id', 'a.role_id','a.status', 'b.last_name'), array('role_id'=>5,'status'=>1), $join);
			 
	//Technical Consultant:	 END				
	// contract technical consultants start
			$contract_tech_consts = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 171,'module_id'=>3));
			$this->data['tech_consts'] = explode(',', $contract_tech_consts[0]['option_value']);
	//contract technical consultants end	
	//Accountant: START
			$join = array(
	    			array(
	    				'table' => 'anb_crm_users_personal_info b',
			...
			$this->data['accountants'] = $this->App_model->getData('anb_crm_users a', array('b.first_name', 'b.user_id', 'a.role_id','a.status', 'b.last_name'), array('role_id'=>6,'status'=>1), $join);
			 
	//Accountant: END				
	// contract technical consultants start
			$acc_status = $this->App_model->getData('anb_crm_record_meta','*', array('id' => 172,'module_id'=>3));
			$this->data['acc_status'] = explode(',', $acc_status[0]['option_value']);
	//contract technical consultants end	
			$this->viewLoad("modules/view");
		}
	...
	public function leads_notes_search()
	{
		$id = $this->input->post('id');
	    $value = $this->input->post('value');
	    $current_record = $this->input->post('current_record');
	   	$data = array('id'=>$id,'value'=>$value,'current_record'=>$current_record);
	   	$result = $this->App_model->leads_notes_search($data);
	   	$ret = array();
	   	foreach ($result as $note) {
	   	     	$add['id'] = $note["id"];
				$add["note"] = $note["note"];
				$add["note_title"] = $note["note_title"];
				$add["created_by"] = ($note["first_name"] . " " . $note["last_name"]);
				$add["created_time"] = date('m/d/Y H:i',$note["created_time"]);
				$ret[] = $add;
	   	}

	   	if($ret)
	   	{ echo json_encode($ret);die; } else	{ echo json_encode('error');die; }
	}
	...
	function create_invoice()
	{
		...
		if($this->data["current_module"]=="Contracts")
		{
			...
			if($_POST)
			{
				...
				if($invoice_id){
                    $this->LogsModel->addLogs('Invoice', 'Invoice is created', $data["record_id"], 'Contracts',$_SESSION['user_id'],'Invoiced');
                }
...
/*		 print_r($data1);
	...
	if($ans == 'Yes')
	{
		$logid = $this->LogsModel->addLogs('Qualified', $current_module.' is Qualified' ,$record_id, $current_module,$_SESSION['user_id'],'Qualify');
	}
	else
	{
		$logid = $this->LogsModel->addLogs('Qualified', $current_module.' is not Qualified' ,$record_id, $current_module,$_SESSION['user_id'],'Qualify');
	}
	if($message && $ans == 'Yes')
	{
	...
	public function reassignedLead()
	{
		...
		if($this->data["current_module"]=="Leads")
		{
			if ( ($requestMethod = strtolower($this->input->method())) == 'get')
			{
				...
				if($status){
					$this->LogsModel->addLogs('Reassigned', $status1, $record_id, 'Leads', $_SESSION['user_id'],'Prospect');;
   ...
   public function getServicesByRecordId()
   {
   	...
   	public function sendMail($action = null, $subject = null, $record_id = null, $heading = null, $slug_name = null){
		...
		if($module_name == 'Clients')
		{
			if(isset($action)){
				$record_data = $this->ModuleModel->loadRecord1(null, $record_id);
					$user_data0 = $this->UserModel->loadUser($record_data['__104']);
					$admin=$this->UserModel->loadUser($record_data['__0']);
					$admin_email=$admin['email'];
					$admin=$admin['first_name'].' '.$admin['last_name'];
					$closer = $user_data0['first_name'].' '.$user_data0['last_name'];
					$closer_email = $user_data0['email'];
					if(!empty($record_data['__106']))
					{
						$technical_consultants_data = $this->UserModel->loadUser($record_data['__106']);
						$technical_consultants = $technical_consultants_data['first_name'].' '.$technical_consultants_data['last_name'];
					}
					if(!empty($record_data['__107']))
					{
						$accountants_data = $this->UserModel->loadUser($record_data['__107']);
						$accountants = $accountants_data['first_name'].' '.$accountants_data['last_name'];
					}
					$toName = $closer;
					$toEmail[0]['email'] = trim($closer_email);
					$template_data = $this->EmailtemplatesModel->get_template_by_slug('email', $slug_name);
					if(!empty($template_data)){
							...
							isset($record_data['__118']) ? $record_data['__118'] : "No Prent Company Name Available",
							
							isset($record_data['__125']) ? $record_data['__125'] : 'No Business Type Available' ,
							isset($record_data['__154']) ? $record_data['__154'] : 'NA',
							isset($record_data['__157']) ? $record_data['__157'] : "NA",
							isset($record_data['__84']) ? $record_data['__84'] : "No Client Name Available",
...
/*				echo $this->email->print_debugger();
	...
	public function getEvents(){
		...
		$bookings= $this->App_model->getData('anb_crm_bookings','booking_date, end_date, booking_title, id, record_id,email,created_for',array('created_for' =>$created_for,  'all_day'=>0));
		...
		for ($i=0; $i <count($bookings); $i++){ 
					$booking = array(
						...
						'created_for'=>$bookings[$i]['created_for'],
						);
	...
	public function getEvents1(){
		...
		$bookings= $this->App_model->getData('anb_crm_bookings','booking_date, end_date, booking_title, id, record_id,email,created_for',array('created_for' =>$user_id,  'all_day'=>0));
		//print_r($bookings); die("ok");	
		for ($i=0; $i <count($bookings); $i++){ 
					$booking = array(
						'id'=>	$bookings[$i]['id'],
	...
	function get_hot_leads(){
       ...
       $role_id = $this->getUserRole();
        ...
        $where = '';
        $select = array('mv.record_id,r.modified_by,c.name,mv.value');
        $order_by = "r.id";
        $temp = 'desc';
        ...
        $where .= "(r.module_id = 1 AND r.record_status = 3 and r.modified_by is not null)";
        ...
        //$this->db->group_by('id');
        $this->db->where($where);
       	$this->db->join('anb_crm_record_meta_value mv','mv.record_id = r.id','left join');
       	$this->db->join('anb_crm_record_meta c','c.id = mv.record_meta_id','left join');

        if (isset($_GET['iDisplayStart']) && $_GET['iDisplayLength'] != '-1') {
            $str_point = intval($_GET['iDisplayStart']);
            $lenght = intval($_GET['iDisplayLength']);
            $records = $this->db->from('anb_crm_record r')
            		->select('mv.record_id,r.modified_by,c.name,mv.value')
       ...
       echo $this->db->last_query();
        ...
        $total_record = $this->db->select('mv.record_id,r.modified_by,c.name,mv.value')
	...
	function get_last_updated_leads(){
		...
		$this->db->where("mod.plural_name",'Leads');
		...
		$this->db->join("(SELECT sq27.value, sq27.record_id FROM anb_crm_record_meta_value sq27 where sq27.record_meta_id = 27) as ad27", "ad27.record_id = r.id", 'left');
	    ...
	    $col_sort = array("r.modified_time",'ad31.value','ad1.value','ad2.value','ad32.value','ad35.value','ad40.value','ad37.value','ad14.value','ad6.value','ad27.value','adopener.first_name','adcloser.first_name','u2.first_name','r.modified_time');

	    $where = '';
	    $select = array('mv.*,r.id,r.record_status');
	    $order_by = "r.modified_time";
	    $temp = 'desc';
	    ...
	    if (isset($_GET['iDisplayStart']) && $_GET['iDisplayLength'] != '-1') {
	        ...
	        $records = $this->db->from('anb_crm_record r')
	        		->distinct()
	        		->select("r.*,ad1.value as fname,ad2.value as lname,ad31.value as company,ad32.value as phone,ad35.value as city,ad40.value as province,ad37.value as country,ad14.value as lead_category,ad6.value as lead_status,ad27.value as call_status,(SELECT CONCAT(adopener.first_name,' ',adopener.last_name)) AS opener,(SELECT CONCAT(adcloser.first_name,' ',adcloser.last_name)) AS closer,(SELECT CONCAT(u2.first_name,' ',u2.last_name)) AS modified_name,u2.first_name,adopener.first_name,adcloser.first_name")
	                ->limit($lenght, $str_point)
	                ->get();
	    } else {
	        $records = $this->db->from('anb_crm_record r')
...
// die("ok");
	    ...
	    if(!empty($result)){
	        ...
	        foreach ($result as $key => $value) {
	    	  ...
	    	  $value['fname']=isset($value['fname'])?$value['fname']:'';
	          $value['lname']=isset($value['lname'])?$value['lname']:''; 
			  $value['Phone']=isset($value['phone'])?$value['phone']:''; 
			  $value['city']=isset($value['city'])?$value['city']:'';  
			  $value['province']=isset($value['province'])?$value['province']:'';  
			  $value['country']=isset($value['country'])?$value['country']:'';  
			  $value['lead_category']=isset($value['lead_category'])?$value['lead_category']:'';  
			  $value['lead_status']=isset($value['lead_status'])?$value['lead_status']:'';  
			  $value["call_status"]=isset($value["call_status"])?$value["call_status"]:'';  
			  $value['opener']=$value['opener']; 
			  $value['closer']=$value['closer'];  
			  
			  $date=!empty($value['modified_time'])?date('m/d/Y H:i',$value["modified_time"]):'-';

			  $output['aaData'][] = array('','<a style="font-weight: bold;" href="'. base_url() .'/modules/viewRecord/?cm=Leads&id='. $value["record_id"] .'&record_status='. $value["record_status"].'">'. $value["company"] .'</a>'
					,$value['fname']
					,$value['lname']
					,$value['phone']
					,$value['city']
	...
	public function addFavoriteList()
	{
		...
		$record_ids = $this->input->post('ids');
		$created_by = $this->session->userdata('user_id');
		$result = array();
		if(!empty($title) && !empty($record_ids))
		{
			$module_id = $mode != 'Leads' ? $mode == 'Clients' ? 2 : 3 : 1;
			$status = $this->App_model->rowInsert('anb_crm_favorite_list', array('title' => $title, 'module_id' => $module_id, 'records' => $record_ids, 'created_by' => $created_by));
			if($status)
			{
				$arr = explode(',', $record_ids);
				foreach ($arr as $key => $value) {
					$records = $this->App_model->getData('anb_crm_record', 'favourite_list_id', array('id' => $value));
					if($records[0]['favourite_list_id'] == '')
					{
						$this->App_model->rowUpdate('anb_crm_record',array('favourite_list_id' => $status), array('id' => $value));
					}
					else
					{
						$data = $records[0]['favourite_list_id'];
						$data = $data.','.$status;
						$this->App_model->rowUpdate('anb_crm_record',array('favourite_list_id' => $data), array('id' => $value));	
					}
				}
				$this->session->set_flashdata('success', 'Favorite list added successfully');
				$result['message'] = 'Favorite list added successfully';
				$result['success'] = true;
			}
			else
			{
	...
	public function editFavoriteList()
	{
		...
		$fav_data=$this->App_model->getData('anb_crm_favorite_list','*',['id'=>$fav_id]);

		$record_ids=$record_ids.','.$fav_data[0]['records'];

		$record_ids=$this->removeDuplicateString($record_ids);

		//echo $record_ids;die();

		if(!empty($fav_id) && !empty($record_ids))
		{
			$module_id = $mode != 'Leads' ? $mode == 'Clients' ? 2 : 3 : 1;

			$status = $this->App_model->rowUpdate('anb_crm_favorite_list', array('records' => $record_ids),array('id'=>$fav_id));
			...
			if($fav_id)
			{
				...
				foreach ($arr as $key => $value) {
					...
					if($records[0]['favourite_list_id'] == '')
					{
						$this->App_model->rowUpdate('anb_crm_record',array('favourite_list_id' => $fav_id), array('id' => $value));
					}
					else
					{
						$data = $records[0]['favourite_list_id'];
				...
				$this->session->set_flashdata('success', 'Favorite list Updated successfully');
				$result['message'] = 'Favorite list Updated successfully';
				$result['success'] = true;
			}
			else
			{
	...
	public function deleteFavoriteList()
	{
		...
		if(!empty($id))
		{
			$status = $this->App_model->rowsDelete('anb_crm_favorite_list', array('id' => $id));
			if($status)
			{
				$this->session->set_flashdata('success', 'Favorite list deleted successfully');
				$result['message'] = 'Favorite list deleted successfully';
				$result['success'] = true;
			}
			else
			{
	...
	public function single_id_deleteFavoriteList()
	{
		$id = $this->input->post("id");
		$fav_id = $this->input->post("fav_id");
		$query = $this->db->query("select * from anb_crm_favorite_list where id = ".$fav_id."");
		$result = $query->result_array();
		$records = $result[0]['records'];
		$array1 = explode(',', $id);
		$array2 = explode(',', $records);
		$array3 = array_diff($array2, $array1);
		$output = implode(',', $array3);

		$query = $this->db->query("update anb_crm_favorite_list set records ='".$output."' where id=".$fav_id."");

		$query1 = $this->db->query("select * from anb_crm_record where id = ".$id."");
		$result1 = $query1->result_array();
		$records = $result1[0]['favourite_list_id'];
		$array1 = explode(',', $fav_id);
		$array2 = explode(',', $records);
		$array3 = array_diff($array2, $array1);
		$output = implode(',', $array3);

		$query1 = $this->db->query("update anb_crm_record set favourite_list_id ='".$output."' where id=".$id."");
	...
	public function removeFavoriteList()
	{
		...
		foreach ($array1 as $value) {
			$query1 = $this->db->query("select * from anb_crm_record where id = ".$value."");
			$result1 = $query1->result_array();
			$records = $result1[0]['favourite_list_id'];
			$array1 = explode(',', $fav_id);
			$array2 = explode(',', $records);
			...
			$query1 = $this->db->query("update anb_crm_record set favourite_list_id ='".$output."' where id=".$value."");
		}
	...
	{
		$contact_meta_id = $this->input->post('contact_meta_id');
		$record_id = $this->input->post('record_id');
		$status = false;
		if(!empty($contact_meta_id) && !empty($record_id))
		{
			$status = $this->App_model->rowUpdate('anb_crm_record', array('primary_contact' => $contact_meta_id), array('id' => $record_id));
   ...
   function get_modules_by_user_id($user_id){
        ...
        $lenght = 10;
        $str_point = 0;
        $keyword=$_GET['alphabet'];
        $fav_list_id=$_GET['fl_id'];
        ...
        if(empty($user_id)){
          $where .= "(r.module_id = $module_id AND r.record_status = 3 AND mv.record_meta_id = $meta_id )";
        }else{
          $where .= "(r.module_id = $module_id AND r.record_status = 3 AND r.assigned_officer_id = $user_id AND mv.record_meta_id = $meta_id )";
        }
        ...
        if(!empty($keyword)){
       		$this->db->like('mv.value', $keyword, 'after'); 
       	}

    	if($module_id==1)
    	{
    		$n_where = 'mv.value >="'.$record_id.'"';
    		$order_by = 'mv.value  asc';
    	}else{
    		$n_where = '1=1';
    		$order_by = 'mv.value asc';
    	}
       ...
       // echo "<pre>";print_r($this->db->last_query());//die("test");

        $this->db->select('mv.*,r.id,r.record_status')
                ->from('anb_crm_record r')
                ->where($where)
                ->join('anb_crm_record_meta_value mv','mv.record_id = r.id');
				...
				if($module_id==1)
    			{
    				$this->db->where('mv.value >=',$record_id);
    			}
        ...
        foreach($result as $lead){
          ...
          $leads_data=$this->ModuleModel->get_modules_data($lead['record_id'],$module_id);
          ...
          foreach ($leads_data as $key => $value) {
            	...
            	if(!empty($value->first_name) && !empty($value->last_name)){
            		...
            		$lead['modified_by']=ucfirst($value->first_name).' '.ucfirst($value->last_name);
                }

            	if(!empty($value->modified_time))
            	    $lead['modified_time']=date('m/d/Y H:i', $value->modified_time);
            	else
            		$lead['modified_time']='';
          } 
     ...
     if(!empty($res)){
	    ...
	    foreach ($res as $key => $value) {
	       ...
	       if ($module_id==1) {
            ...
            $company_website= isset($value["Company Website"])?$value["Company Website"]:'';
            $output['aaData'][] = array('','<a href="'. base_url() .'/modules/viewRecord/?cm=Leads&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Company Name"] .'</a>'
					,
					'<a href="'. base_url() .'/modules/viewRecord/?cm=Leads&id='. $value["id"] .'&record_status='. $value["record_status"].'">'.$value['First Name'].'</a>',
					 '<a href="'. base_url() .'/modules/viewRecord/?cm=Leads&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Last Name"] .'</a>'
					,'<a href="'. base_url() .'/modules/viewRecord/?cm=Leads&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Direct Number"] .'</a>'
					,'<a href="'. base_url() .'/modules/viewRecord/?cm=Leads&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Province"] .'</a>'
					,'<a href="'. base_url() .'/modules/viewRecord/?cm=Leads&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Country"] .'</a>'
			        ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Leads&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["City"] .'</a>'
			        ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Leads&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Estimated Sales"] .'</a>'
			        ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Leads&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Reported Sales"] .'</a>'
			         ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Leads&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["modified_by"] .'</a>'
			         ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Leads&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value['modified_time'] .'</a>'
			          ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Leads&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Lead Status"] .'</a>');	

	       }	

	       if($module_id==2) {
	                 
		       $value["Client Name"]=isset($value["Client Name"])?$value["Client Name"]:'';
	           $value["City"]=isset($value["City"])?$value["City"]:'';
	           $value["Province"]=isset($value["Province"])?$value["Province"]:'';
	           $value["Country"]=isset($value["Country"])?$value["Country"]:'';
	           $value["First Name"]=isset($value["First Name"])?$value["First Name"]:'';
	           ...
	           $value["Phone"]=isset($value["Phone"])?$value["Phone"]:'';
	           $value["Fax"]=isset($value["Fax"])?$value["Fax"]:'';
	           $value["Email"]=isset($value["Email"])?$value["Email"]:'';
	           $value["Client Status"]=isset($value["Client Status"])?$value["Client Status"]:'';
	           $value["Service Type"]=isset($value["Service Type"])?$value["Service Type"]:'';
	           $value["Estimated Sales"]=isset($value["Estimated Sales"])?$value["Estimated Sales"]:'';
	           $value["Primary SIC Code"]=isset($value["Primary SIC Code"])?$value["Primary SIC Code"]:'';
	           $value["Signing Rate"]=isset($value["Signing Rate"])?$value["Signing Rate"]:'';
	           $value["Master Contract Number"]=isset($value["Master Contract Number"])?$value["Master Contract Number"]:'';

	           $output['aaData'][] = array('','<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Client Name"] .'</a>'
				 ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["City"] .'</a>'
				,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Province"] .'</a>'
				,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Country"] .'</a>'
	             ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'.$value['First Name'].'</a>'
				,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Last Name"] .'</a>'
				,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Title"] .'</a>'
				,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Phone"] .'</a>'
		        ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Fax"] .'</a>'
		         ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Email"] .'</a>'
		         ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value['Client Status'] .'</a>'
		         ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Service Type"] .'</a>'
		         ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Estimated Sales"] .'</a>'
		         ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Primary SIC Code"] .'</a>'
		         ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Signing Rate"] .'</a>'
		         ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Clients&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Master Contract Number"] .'</a>');	   		
	         

	        } 
	        if ($module_id==3) {
	             
               $value["Contract Client"]=isset($value["Contract Client"])?$value["Contract Client"]:'';
	           $value["Client Number"]=isset($value["City"])?$value["Client Number"]:'';
	           $value["Contract Name"]=isset($value["Contract Name"])?$value["Contract Name"]:'';
	           $value["Contract Number"]=isset($value["Contract Number"])?$value["Contract Number"]:'';
	           $value["Signing Rate"]=isset($value["Signing Rate"])?$value["Signing Rate"]:'';
	           $value["Closer Name"]=isset($value["Closer Name"])?$value["Closer Name"]:'';
	           $value["Closing Date"]=isset($value["Closing Date"])?$value["Closing Date"]:'';
	           $value["Technical Consultant"]=isset($value["Technical Consultant"])?$value["Technical Consultant"]:'';
	           $value["Assigned Officer"]=isset($value["Assigned Officer"])?$value["Assigned Officer"]:'';
	           $value["Assigned Date"]=isset($value["Assigned Date"])?$value["Assigned Date"]:'';
	           $value["Completed On"]=isset($value["Completed On"])?$value["Completed On"]:'';
	           $value["Stage"]=isset($value["Stage"])?$value["Stage"]:'';
	           $value["Technical Expected Start Date"]=isset($value["Technical Expected Start Date"])?$value["Technical Expected Start Date"]:'';
	           $value["Service Type"]=isset($value["Service Type"])?$value["Service Type"]:'';
	           

	           $output['aaData'][] = array('','<a href="'. base_url() .'/modules/viewRecord/?cm=Contracts&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Contract Client"] .'</a>'
				 ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Contracts&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Client Number"] .'</a>'
				,'<a href="'. base_url() .'/modules/viewRecord/?cm=Contracts&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Contract Name"] .'</a>'
				,'<a href="'. base_url() .'/modules/viewRecord/?cm=Contracts&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Contract Number"] .'</a>'
	             ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Contracts&id='. $value["id"] .'&record_status='. $value["record_status"].'">'.$value['Signing Rate'].'</a>'
				,'<a href="'. base_url() .'/modules/viewRecord/?cm=Contracts&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Closer Name"] .'</a>'
				,'<a href="'. base_url() .'/modules/viewRecord/?cm=Contracts&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Closing Date"] .'</a>'
				,'<a href="'. base_url() .'/modules/viewRecord/?cm=Contracts&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Technical Consultant"] .'</a>'
		        ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Contracts&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Assigned Officer"] .'</a>'
		         ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Contracts&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Assigned Date"] .'</a>'
		         ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Contracts&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value['Completed On'] .'</a>'
		         ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Contracts&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Stage"] .'</a>'
		         ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Contracts&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Technical Expected Start Date"] .'</a>'
		         ,'<a href="'. base_url() .'/modules/viewRecord/?cm=Contracts&id='. $value["id"] .'&record_status='. $value["record_status"].'">'. $value["Service Type"] .'</a>');
 ...
 public function restore(){
 	$record_id=$this->input->post('record_id');
 	echo $this->ModuleModel->restore($record_id);die();
 }    
 ...
 public function import()
 {
 	...
 	die('you already imported all records');

 	ini_set('max_execution_time', '0');
 	
 	echo '<pre>';
 	echo 'Record inserted after 37413 <br>';

    $path=base_url('assets/anb_excel_data.csv');
	$file = fopen($path,"r");
    $i=1;
	while(! feof($file))
	{
	    ...
	    if ($i > 4) {
	    	...
	    	$info['zoom_id']=preg_replace('/-/', '',$fdata[0]);
	    	
	    	$info['modified_time']=strtotime($fdata[13]);

	    	$array['/modules/addRecord/'] =''; 
		    $array['cm'] = 'Leads';
		    $array['ac'] = 'new';
            $array['module_name'] = 'Leads';
		    $array['__0'] = 1;
		    
            $name=preg_split("/ /", $fdata[1]);
          
		    $array['__1'] = $name[0];
		    $array['__2'] = $name[1].' '.$name[2];
		    ...
		    if (preg_match("/\bext\b/i", $fdata[5], $match,PREG_OFFSET_CAPTURE))  
		    {
		    	//print_r($match);
		    	$number=str_split($fdata[5], $match[0][1]);
		    	$ext=preg_split("/ /",$number[1]);
		    	$array['__19'] =$number[0];
		    	$array['__15'] =$ext[1];
		    }
			else{
		    ...
		    $array['__200'] = $fdata[8]; 
		    $array['__201'] = $fdata[9]; 
		    $array['__202'] = $fdata[10];
		    $array['__203'] = $fdata[11];
		    $array['__204'] = $fdata[12];
		    $array['__21'] = 1;
		    $array['__22'] =1; 
		    $array['__24'] = '';
		    $array['__26'] = 'None';
		    ...
		    $array['__75'] = ''; 
		    $array['__76'] =$fdata[30]; 
		    $array['__206'] = $fdata[26]; 
		    $array['__77'] =''; 
		    $array['__207'] =$fdata[28]; 
		    $array['__78'] = '';
		    $array['__205'] =$fdata[25]; 
		    $array['__208'] =$fdata[27]; 
		    $array['__79'] = 'None';
		    $array['__80'] =''; 
		    $array['__82'] =$fdata[29]; 
		    $array['__209'] = $fdata[31];
		    $array['__210'] =$fdata[32]; 

            $this->data['meta_fields']=$this->ModuleModel->loadModulesMeta('Leads');

           $res = $this->ModuleModel->import_data($this->data['meta_fields'],$this->getUserId(),$array,$info);
 ...
 public function phone()
 {
 	 ...
 	 die('you already updated phone format');
 	 ...
 	 $this->db->group_start();
 	 $this->db->where('value','USA');
 	 $this->db->or_where('value','United States');
 	 $this->db->or_where('value','Canada');
 	 $this->db->group_end();
     ...
     foreach ($res as $key => $value) {
		 ...
		 $this->db->or_where('record_meta_id',33);
		 $this->db->or_where('record_meta_id',109);
		 $this->db->or_where('record_meta_id',110);
		 $this->db->group_end();
     	 ...
     	 foreach ($data as $key => $phone) {
     	 	  if(!empty($phone->value)){
     	 	  	  ...
     	 	  	  if(strpos($phone->value, '(') === false){
     	 	  	  	   $str=str_replace('+','',$phone->value);
                       if (substr_count($str, "-")==2) {
                       	    
                       	  $array=explode('-',$str);
                       	  
                       	  $newstr='('.$array[0].') '.$array[1].'-'.$array[2];

                       	  if($this->db->update('anb_crm_record_meta_value',['value'=>$newstr],['id'=>$phone->id]))
                          {

                       	  	echo $phone->id.'--new'.$newstr.'--old--'.$phone->value.'--Updated<br>';
                          }else{
                          	 echo 'Error<br>';
                          }
 ...
 public function edit_by_individual_tab()
 {
 		$loggedUserId = $this->getUserId();
	 	$r_id = $this->input->post('record_id');
	 	$id = array("id"=>$r_id,"login_userid"=>$loggedUserId);

	 	 $this->db->where('id', $r_id);	
	     $this->db->update('anb_crm_record', array('modified_by'=>$loggedUserId)); 

 	    $detail = $this->input->post('detail_set');
...
//  	die("ok");
 	    if(isset($detail) == "detail")
 	    {
		 	...
		 	$aa = $this->input->post('__1');
		 	$bb = $this->input->post('__2');
		 	$cc = $this->input->post('__3');
		 	$dd = $this->input->post('__8');
		 	$ee = $this->input->post('__19');
		 	$ff = $this->input->post('__15');
		 	// $gg = $this->input->post('__20');
		 	$hh = $this->input->post('__4');
		 	
		 	$data = array("1"=>$aa,"2"=>$bb,"3"=>$cc,"8"=>$dd,"19"=>$ee,"15"=>$ff,"4"=>$hh);
		 // var_dump($id);	echo "<pre>"; print_r($data);die("ok");
		 	$result = $this->ModuleModel->edit_by_individual_tab($id,$data);
		 	if($result)
		 	{echo json_encode("success");die;}else{echo json_encode("error");die;}
	 	}

	 	 $person2 = $this->input->post('person2');
 	    if(isset($person2) == "person2")
 	    {
 	    	$aa = $this->input->post('__50');
		 	$bb = $this->input->post('__51');
		 	$cc = $this->input->post('__52');
		 	$dd = $this->input->post('__53');
		 	$ee = $this->input->post('__54');
		 	$ff = $this->input->post('__55');
		 	$gg = $this->input->post('__56');

		 	$data = array("50"=>$aa,"51"=>$bb,"52"=>$cc,"53"=>$dd,"54"=>$ee,"55"=>$ff,"56"=>$gg);
 	    ...
 	    if(isset($person3) == "person3")
 	    {
 	    	$aa = $this->input->post('__57');
		 	$bb = $this->input->post('__58');
		 	$cc = $this->input->post('__59');
		 	$dd = $this->input->post('__60');
		 	$ee = $this->input->post('__61');
		 	$ff = $this->input->post('__62');
		 	$gg = $this->input->post('__63');

		 	$data = array("57"=>$aa,"58"=>$bb,"59"=>$cc,"60"=>$dd,"61"=>$ee,"62"=>$ff,"63"=>$gg);
 	    ...
 	    if(isset($person4) == "person4")
 	    {
 	    	$aa = $this->input->post('__64');
		 	$bb = $this->input->post('__65');
		 	$cc = $this->input->post('__66');
		 	$dd = $this->input->post('__67');
		 	$ee = $this->input->post('__68');
		 	$ff = $this->input->post('__69');
		 	$gg = $this->input->post('__70');

		 	$data = array("64"=>$aa,"65"=>$bb,"66"=>$cc,"67"=>$dd,"68"=>$ee,"69"=>$ff,"70"=>$gg);
 	    ...
 	    if(isset($person5) == "person5")
 	    {
 	    	$aa = $this->input->post('__71');
		 	$bb = $this->input->post('__72');
		 	$cc = $this->input->post('__73');
		 	$dd = $this->input->post('__74');
		 	$ee = $this->input->post('__75');
		 	$ff = $this->input->post('__245');
		 	$gg = $this->input->post('__244');

		 	$data = array("71"=>$aa,"72"=>$bb,"73"=>$cc,"74"=>$dd,"75"=>$ee,"245"=>$ff,"244"=>$gg);
 	    ...
 	    $general_company = $this->input->post('general_company');
 	    if(isset($general_company) == "general_company")
 	    {

		 	$aa = $this->input->post('__31');
		 	$bb = $this->input->post('__42');
		 	$cc = $this->input->post('__38');
		 	$dd = $this->input->post('__39');
		 	$ee = $this->input->post('__35');
		 	$ff = $this->input->post('__40');
		 	$gg = $this->input->post('__37');
		 	$hh = $this->input->post('__36');
		 	$ii = $this->input->post('__32');
		 	$jj = $this->input->post('__33');
		 	$kk = $this->input->post('__34');
	
		 	$data = array("31"=>$aa,"42"=>$bb,"38"=>$cc,"39"=>$dd,"35"=>$ee,"40"=>$ff,"37"=>$gg,"36"=>$hh,"32"=>$ii,"33"=>$jj,"34"=>$kk);
		 	$result = $this->ModuleModel->edit_by_individual_tab($id,$data);
		 	if($result)
		 	{echo json_encode("success");die;}else{echo json_encode("error");die;}
	 	}
 	    ...
 	    if(isset($parent_company) == "parent_company")
 	    {
		 	...
		 	$aa = $this->input->post('__41');
		 	$bb = $this->input->post('__43');
		 	$cc = $this->input->post('__45');
		 	$dd = $this->input->post('__44');
		 	$ee = $this->input->post('__46');
		 	$ff = $this->input->post('__47');
		 	$gg = $this->input->post('__186');
		 	$hh = $this->input->post('__187');
		 	$ii = $this->input->post('__188');
		 	$jj = $this->input->post('__189');

		 	$data = array("41"=>$aa,"43"=>$bb,"45"=>$cc,"44"=>$dd,"46"=>$ee,"47"=>$ff,"186"=>$gg,"187"=>$hh,"188"=>$ii,"189"=>$jj);
	 	...
	 	$additional_company = $this->input->post('additional_company');
 	    if(isset($additional_company) == "additional_company")
 	    {
		 	$aa = $this->input->post('__48');
		 	$bb = $this->input->post('__49');
		 	$cc = $this->input->post('__80');
		 	$dd = $this->input->post('__82');
		 	$ee = $this->input->post('__76');
		 	$ff = $this->input->post('__206');
		 	$gg = $this->input->post('__77');
		 	$hh = $this->input->post('__78');
		 	$ii = $this->input->post('__81');
		 	$jj = $this->input->post('__79');
		 	$ll = $this->input->post('__9');
		 	$mm = $this->input->post('__190');
		    $nn = $this->input->post('__209');
		    $oo = $this->input->post('__210');

		 	$kk = $this->input->post('__18');
		 	if(!empty($kk))
		 	{ $kk = implode("____",$kk); }
		 	$ll = $this->input->post('__9');
		 	if(!empty($ll))
		 	{ $ll = implode("____",$ll); }
		    $pp = $this->input->post('__17');
		    if(!empty($pp))
		 	{ $pp = implode("____",$pp);  }

		 	$data = array("48"=>$aa,"49"=>$bb,"80"=>$cc,"82"=>$dd,"76"=>$ee,"206"=>$ff,"77"=>$gg,"78"=>$hh,"81"=>$ii,"79"=>$jj,"18"=>$kk,"9"=>$ll,"190"=>$mm,"209"=>$nn,"210"=>$oo,"17"=> $pp);
		 	// echo "<pre>";print_r($data);die();
		 	$result = $this->ModuleModel->edit_by_individual_tab($id,$data);
		 	if($result)
		 	{echo json_encode("success");die;}else{echo json_encode("error");die;}
	 	...
	 	$lead_processor = $this->input->post('lead_processor');
 	    if(isset($lead_processor) == "lead_processor")
 	    {	
 	    	

		 	$aa = $this->input->post('__32');
		 	$bb = $this->input->post('__4');
		 	$cc = $this->input->post('__27');
		 	$dd = $this->input->post('__6');
		 	$ee = $this->input->post('__14');

		 	$ff = $this->input->post('__21');
		 	$gg = $this->input->post('__22');

		 	$ii = $this->input->post('__13');
		 	$jj = $this->input->post('__5');
		 	$kk = $this->input->post('__11');
		 	$ll = $this->input->post('__7');
		 	$mm = $this->input->post('__10');
		 	$nn = $this->input->post('__12');
		 	$oo = $this->input->post('__16');

		 	$hh = $this->input->post('__9');
		 	if(!empty($hh))
		 	{ $hh = implode("____",$hh); }

		 	//for opener
		 	         $this->db->where('id', $r_id);	
	                 $this->db->update('anb_crm_record', array('assigned_officer_id'=>$gg)); 
	         //end        
		 	$data = array("32"=>$aa,"4"=>$bb,"27"=>$cc,"6"=>$dd,"14"=>$ee,"21"=>$ff,"22"=>$gg,"9"=>$hh,"13"=>$ii,"5"=>$jj,"11"=>$kk,"7"=>$ll,"10"=>$mm,"12"=>$nn,"16"=>$oo);
		 	$result = $this->ModuleModel->edit_by_individual_tab($id,$data);
		 	            $heading = 'Lead Assigned';
						$subject = 'Lead is assigned to you';
						$slug_name = 'admin_to_opener';
		 	$status = $this->sendMail('mOwner', $subject, $r_id , $heading, $slug_name);
		 	//var_dump($status);
		 	if($result)
		 	{echo json_encode("success");die;}else{echo json_encode("error");die;}
	 	}
  ...
  public function client_edit_by_individual_tab()
	 {
	 	$loggedUserId = $this->getUserId();
	 	$r_id = $this->input->post('record_id');
	 	$id = array("id"=>$r_id,"login_userid"=>$loggedUserId);
 	    ...
 	    if(isset($detail) == "detail")
 	    {
		 	...
		 	$aa = $this->input->post('__184');
		 	$bb = $this->input->post('__183');
		 	$cc = $this->input->post('__83');
		 	$dd = $this->input->post('__89');
		 	$ee = $this->input->post('__100');
		 	$ff = $this->input->post('__96');
		 	$gg = $this->input->post('__103');
		 	$hh = $this->input->post('__85');
		 	
		 	$data = array("184"=>$aa,"183"=>$bb,"83"=>$cc,"89"=>$dd,"100"=>$ee,"96"=>$ff,"103"=>$gg,"85"=>$hh);
 	    ...
 	    if(isset($person2) == "person2")
 	    {
 	    	$aa = $this->input->post('__127');
		 	$bb = $this->input->post('__128');
		 	$cc = $this->input->post('__129');
		 	$dd = $this->input->post('__130');
		 	$ee = $this->input->post('__131');
		 	$ff = $this->input->post('__132');
		 	$gg = $this->input->post('__133');

		 	$data = array("127"=>$aa,"128"=>$bb,"129"=>$cc,"130"=>$dd,"131"=>$ee,"132"=>$ff,"133"=>$gg);
 	    ...
 	    if(isset($person3) == "person3")
 	    {
 	    	$aa = $this->input->post('__134');
		 	$bb = $this->input->post('__135');
		 	$cc = $this->input->post('__136');
		 	$dd = $this->input->post('__137');
		 	$ee = $this->input->post('__138');
		 	$ff = $this->input->post('__139');
		 	$gg = $this->input->post('__140');

		 	$data = array("134"=>$aa,"135"=>$bb,"136"=>$cc,"137"=>$dd,"138"=>$ee,"139"=>$ff,"140"=>$gg);
 	    ...
 	    if(isset($person4) == "person4")
 	    {
 	    	$aa = $this->input->post('__141');
		 	$bb = $this->input->post('__142');
		 	$cc = $this->input->post('__143');
		 	$dd = $this->input->post('__144');
		 	$ee = $this->input->post('__145');
		 	$ff = $this->input->post('__146');
		 	$gg = $this->input->post('__147');

		 	$data = array("141"=>$aa,"142"=>$bb,"143"=>$cc,"144"=>$dd,"145"=>$ee,"146"=>$ff,"147"=>$gg);
 	    ...
 	    if(isset($person5) == "person5")
 	    {
 	    	$aa = $this->input->post('__148');
		 	$bb = $this->input->post('__149');
		 	$cc = $this->input->post('__150');
		 	$dd = $this->input->post('__151');
		 	$ee = $this->input->post('__152');
			$ff = $this->input->post('__246');
		 	$gg = $this->input->post('__247');

		 	$data = array("148"=>$aa,"149"=>$bb,"150"=>$cc,"151"=>$dd,"152"=>$ee,"246"=>$ff,"247"=>$gg);
 	    ...
 	    if(isset($general_company) == "general_company")
 	    {
		 	...
		 	$aa = $this->input->post('__108');
		 	$bb = $this->input->post('__109');
		 	$cc = $this->input->post('__110');
		 	$dd = $this->input->post('__111');
		 	$ee = $this->input->post('__112');
		 	$ff = $this->input->post('__113');
		 	$gg = $this->input->post('__114');
		 	$hh = $this->input->post('__115');
		 	$ii = $this->input->post('__116');
		 	$jj = $this->input->post('__117');
		 	$kk = $this->input->post('__119');
	
		 	$data = array("108"=>$aa,"109"=>$bb,"110"=>$cc,"111"=>$dd,"112"=>$ee,"113"=>$ff,"114"=>$gg,"115"=>$hh,"116"=>$ii,"117"=>$jj,"119"=>$kk);
 	    ...
 	    if(isset($parent_company) == "parent_company")
 	    {
		 	...
		 	$aa = $this->input->post('__118');
		 	$bb = $this->input->post('__120');
		 	$cc = $this->input->post('__121');
		 	$dd = $this->input->post('__122');
		 	$ee = $this->input->post('__123');
		 	$ff = $this->input->post('__124');
		 	$gg = $this->input->post('__186');
		 	$hh = $this->input->post('__187');
		 	$ii = $this->input->post('__188');
		 	$jj = $this->input->post('__189');

		 	$data = array("118"=>$aa,"120"=>$bb,"121"=>$cc,"122"=>$dd,"123"=>$ee,"124"=>$ff,"186"=>$gg,"187"=>$hh,"188"=>$ii,"189"=>$jj);
 	    ...
 	    if(isset($additional_company) == "additional_company")
 	    {
		 	$aa = $this->input->post('__125');
		 	$bb = $this->input->post('__126');
		 	$cc = $this->input->post('__157');
		 	$dd = $this->input->post('__159');
		 	$ee = $this->input->post('__153');
		 	$ff = $this->input->post('__218');
		 	$gg = $this->input->post('__154');
		 	$hh = $this->input->post('__155');
		 	$ii = $this->input->post('__158');
		 	$jj = $this->input->post('__79');
		    $nn = $this->input->post('__190');
		    $oo = $this->input->post('__191');
		    $qq = $this->input->post('__192');
		    $rr = $this->input->post('__193');
		    $ss = $this->input->post('__194');
		    $tt = $this->input->post('__195');
		    $uu = $this->input->post('__221');
		    $vv = $this->input->post('__222');

		 	$kk = $this->input->post('__98');
		 	if(!empty($kk))
		 	{ $kk = implode("____",$kk); }
		 	$ll = $this->input->post('__99');
		 	if(!empty($ll))
		 	{ $ll = implode("____",$ll); }
		 	$mm = $this->input->post('__90');
		    if(!empty($mm))
		 	{ $mm = implode("____",$mm);  }

		 	$data = array("125"=>$aa,"126"=>$bb,"157"=>$cc,"159"=>$dd,"153"=>$ee,"218"=>$ff,"154"=>$gg,"155"=>$hh,"158"=>$ii,"79"=>$jj,"98"=>$kk,"99"=>$ll,"90"=>$mm,"190"=>$nn,"191"=>$oo,"192"=> $qq,"193"=> $rr,"194"=> $ss,"195"=> $tt,"221"=> $uu,"222"=> $vv);
		 	$result = $this->ModuleModel->edit_by_individual_tab($id,$data);
		 	if($result)
		 	{echo json_encode("success");die;}else{echo json_encode("error");die;}
	 	}
 	    ...
 	    {	
		 	...
		 	$aa = $this->input->post('__109');
		 	$bb = $this->input->post('__85');
		 	$cc = $this->input->post('__87');
		 	$dd = $this->input->post('__95');

		 	$ee = $this->input->post('__104');
		 	$ff = $this->input->post('__105');

		 	$gg = $this->input->post('__97');
		 	$ii = $this->input->post('__86');
		 	$jj = $this->input->post('__92');
		 	$kk = $this->input->post('__88');
		 	$ll = $this->input->post('__91');
		 	$mm = $this->input->post('__93');
		 	
		 	$hh = $this->input->post('__90');
		    if(!empty($hh))
		 	{ $hh = implode("____",$hh);  }
		 
		 	//for opener
		 	         $this->db->where('id', $r_id);	
	                 $this->db->update('anb_crm_record', array('assigned_officer_id'=>$ee)); 
	         //end        
		 	$data = array("109"=>$aa,"85"=>$bb,"87"=>$cc,"95"=>$dd,"104"=>$ee,"105"=>$ff,"97"=>$gg,"90"=>$hh,"86"=>$ii,"92"=>$jj,"88"=>$kk,"91"=>$ll,"93"=>$mm);
		 	$result = $this->ModuleModel->edit_by_individual_tab($id,$data);
		 	            $heading = 'Client Assigned';
						$subject = 'Client is assigned to you';
						$slug_name = 'Admin_to_Closer';
	 ...
	 public function contract_edit_by_individual_tab()
	 {
	 	$loggedUserId = $this->getUserId();
	 	$r_id = $this->input->post('record_id');
	 	$id = array("id"=>$r_id,"login_userid"=>$loggedUserId);
 	    ...
 	    if(isset($detail) == "detail")
 	    {
		 	$aa = $this->input->post('__161');
		 	$bb = $this->input->post('__164');
		 	$cc = $this->input->post('__166');
		 	$dd = $this->input->post('__167');
		 	
		 	$contract_owner = $this->input->post("contract_owner");
		 	//for opener
		 	         $this->db->where('id', $r_id);	
	                 $this->db->update('anb_crm_record', array('assigned_officer_id'=>$contract_owner)); 
	         //end  
		 	$data = array("161"=>$aa,"164"=>$bb,"166"=>$cc,"167"=>$dd);
		 	$result = $this->ModuleModel->edit_by_individual_tab($id,$data);
				 	    $heading = 'Contract Assigned';
						$subject = 'New Contract is Added';
						$slug_name = 'new_contract_client';
		 	$status = $this->sendMail('cOwner', $subject, $r_id , $heading, $slug_name);
		 	//var_dump($status);
		 	if($result)
		 	{echo json_encode("success");die;}else{echo json_encode("error");die;}
	 	}

  		$contract_info = $this->input->post('contract_info');
 	    if(isset($contract_info) == "contract_info")
 	    {
 	    	$aa = $this->input->post('__162');
		 	$cc = $this->input->post('__83');
		 	$dd = $this->input->post('__89');
		 	$ee = $this->input->post('__100');
			$ff = $this->input->post('__96');
			...
			$hh = $this->input->post('__85');
		 	...
		 	$bb = $this->input->post('__163');
		    if(!empty($bb))
		 	{ $bb = implode("____",$bb);  }

		 	$data = array("162"=>$aa,"163"=>$bb,"83"=>$cc,"89"=>$dd,"100"=>$ee,"96"=>$ff,"103"=>$gg,"85"=>$hh);
		 	// echo "<pre>";print_r($data);die;
		 	$result = $this->ModuleModel->edit_by_individual_tab($id,$data);
 	    ...
 	    $contract_pro_info = $this->input->post('contract_pro_info');
 	    if(isset($contract_pro_info) == "contract_pro_info")
 	    {
 	    	$aa = $this->input->post('__169');
		 	$bb = $this->input->post('__170');
		 	$cc = $this->input->post('__171');
		 	$dd = $this->input->post('__172');
		 	$ee = $this->input->post('__173');
			$ff = $this->input->post('__174');
			$gg = $this->input->post('__175');
			$hh = $this->input->post('__176');
			$ii = $this->input->post('__177');

		 	$data = array("169"=>$aa,"170"=>$bb,"171"=>$cc,"172"=>$dd,"173"=>$ee,"174"=>$ff,"175"=>$gg,"176"=>$hh,"177"=>$ii);
		 	// echo "<pre>";print_r($data);die;
		 	$result = $this->ModuleModel->edit_by_individual_tab($id,$data);
		 	if($result)
		 	{ echo json_encode("success");die;}else{ echo json_encode("error");die;}
 	    }
 	     $contract_d_info = $this->input->post('contract_d_info');
 	    if(isset($contract_d_info) == "contract_d_info")
 	    {
 	    	$aa = $this->input->post('__181');
		 	$bb = $this->input->post('__196');

		 	$data = array("181"=>$aa,"196"=>$bb);
		 	$result = $this->ModuleModel->edit_by_individual_tab($id,$data);
		 	if($result)
		 	{ echo json_encode("success");die;}else{ echo json_encode("error");die;}
 	    }
	 ...
	 {
	 	$r_id = $this->input->post("record_id");
	 	$meta_id = $this->input->post("contact_meta_id");
	 	$loggedUserId = $this->getUserId();
	 	$id = array("id"=>$r_id,"login_userid"=>$loggedUserId);
		$result = $this->ModuleModel->loadRecord1(null, $r_id);

	    	$d_firstname = $result['__1'];
		 	$d_last_name = $result['__2'];
		 	$d_title = $result['__3'];
		 	$d_phone = $result['__19'];
		 	$d_phone_ext = $result['__15'];
		 	$d_function = $result['__8'];
		 	$d_email = $result['__4'];	

	 	if($meta_id == 51)
	 	{

		 	$p1_firstname = $result['__51'];
		 	$p1_last_name = $result['__50'];
		 	$p1_title = $result['__53'];
		 	$p1_phone = $result['__55'];
		 	$p1_phone_ext = $result['__56'];
		 	$p1_function = $result['__52'];
		 	$p1_email = $result['__54'];	

		 	$data = array("50"=>$d_last_name,"51"=>$d_firstname,"52"=>$d_function,"53"=>$d_title,"54"=>$d_email,"55"=>$d_phone,"56"=>$d_phone_ext);
//echo "<pre>";var_dump($r_id);print_r($data);die;
		 	$this->ModuleModel->edit_by_individual_tab($id,$data);

		 	$data_d = array("2"=>$p1_last_name,"1"=>$p1_firstname,"8"=>$p1_function,"3"=>$p1_title,"4"=>$p1_email,"19"=>$p1_phone,"15"=>$p1_phone_ext);

		 	$data_result = $this->ModuleModel->edit_by_individual_tab($id,$data_d);
		 	if($data_result)
		 	{ echo json_encode("success");die;}else{ echo json_encode("error");die;}
	 	...
	 	if($meta_id == 60)
	 	{
		 	$p2_firstname = $result['__60'];
		 	$p2_last_name = $result['__58'];
		 	$p2_title = $result['__57'];
		 	$p2_phone = $result['__62'];
		 	$p2_phone_ext = $result['__63'];
		 	$p2_function = $result['__59'];
		 	$p2_email = $result['__61'];	

		 	$data = array("58"=>$d_last_name,"60"=>$d_firstname,"59"=>$d_function,"57"=>$d_title,"61"=>$d_email,"62"=>$d_phone,"63"=>$d_phone_ext);

			$data_d = array("2"=>$p2_last_name,"1"=>$p2_firstname,"8"=>$p2_function,"3"=>$p2_title,"4"=>$p2_email,"19"=>$p1_phone,"15"=>$p2_phone_ext);
	 	...
	 	if($meta_id == 64)
	 	{	
		 	$p3_firstname = $result['__64'];
		 	$p3_last_name = $result['__66'];
		 	$p3_title = $result['__65'];
		 	$p3_phone = $result['__69'];
		 	$p3_phone_ext = $result['__70'];
		 	$p3_function = $result['__67'];
		 	$p3_email = $result['__68'];	

		 	$data = array("66"=>$d_last_name,"64"=>$d_firstname,"67"=>$d_function,"65"=>$d_title,"68"=>$d_email,"69"=>$d_phone,"70"=>$d_phone_ext);

		 	$data_d = array("2"=>$p3_last_name,"1"=>$p3_firstname,"8"=>$p3_function,"3"=>$p3_title,"4"=>$p3_email,"19"=>$p3_phone,"15"=>$p3_phone_ext);
	 	...
	 	if($meta_id == 71)
	 	{
		 	$p4_firstname = $result['__71'];
		 	$p4_last_name = $result['__73'];
		 	$p4_title = $result['__72'];
		 	$p4_phone = $result['__75'];
		 	$p4_phone_ext = $result['__245'];
		 	$p4_function = $result['__244'];
		 	$p4_email = $result['__74'];	

		 	$data = array("73"=>$d_last_name,"71"=>$d_firstname,"72"=>$d_title,"74"=>$d_email,"75"=>$d_phone,"245"=>$d_phone_ext,"244"=>$d_function);

		 	    $this->ModuleModel->edit_by_individual_tab($id,$data);

		 	$data_d = array("2"=>$p4_last_name,"1"=>$p4_firstname,"8"=>$p4_function,"3"=>$p4_title,"4"=>$p4_email,"19"=>$p4_phone,"15"=>$p4_phone_ext);
	 ...
	 public function update_tab_client()
	 {
	 	$r_id = $this->input->post("record_id");
	 	$meta_id = $this->input->post("contact_meta_id");
	 	$loggedUserId = $this->getUserId();
		...
		$result = $this->ModuleModel->loadRecord1(null, $r_id);
	    	...
	    	$d_firstname = $result['__184'];
		 	$d_last_name = $result['__183'];
		 	$d_function = $result['__89'];
		 	$d_title = $result['__83'];
		 	$d_email = $result['__85'];	
		 	$d_phone = $result['__100'];
		 	$d_phone_ext = $result['__96'];

	 //echo "<pre>";print_r($meta_id);die("test");

	 	if($meta_id == 128)
	 	{

		 	$p1_last_name = $result['__127'];
		 	$p1_firstname = $result['__128'];
		 	$p1_function = $result['__129'];
		 	$p1_title = $result['__130'];
		 	$p1_phone = $result['__132'];
		 	$p1_phone_ext = $result['__133'];
		 	$p1_email = $result['__131'];	

		 	$data = array("127"=>$d_last_name,"128"=>$d_firstname,"129"=>$d_function,"130"=>$d_title,"131"=>$d_email,"132"=>$d_phone,"133"=>$d_phone_ext);

		 	$this->ModuleModel->edit_by_individual_tab($id,$data);

		 	$data_d = array("183"=>$p1_last_name,"184"=>$p1_firstname,"89"=>$p1_function,"83"=>$p1_title,"85"=>$p1_email,"100"=>$p1_phone,"96"=>$p1_phone_ext);
	 	...
	 	if($meta_id == 137)
	 	{
		 	$p2_firstname = $result['__137'];
		 	$p2_last_name = $result['__135'];
		 	$p2_title = $result['__134'];
		 	$p2_phone = $result['__139'];
		 	$p2_phone_ext = $result['__140'];
		 	$p2_function = $result['__136'];
		 	$p2_email = $result['__138'];	

		 	$data = array("135"=>$d_last_name,"137"=>$d_firstname,"136"=>$d_function,"134"=>$d_title,"138"=>$d_email,"139"=>$d_phone,"140"=>$d_phone_ext);

		 	$data_d = array("183"=>$p2_last_name,"184"=>$p2_firstname,"89"=>$p2_function,"83"=>$p2_title,"85"=>$p2_email,"100"=>$p2_phone,"96"=>$p2_phone_ext);
	 	...
	 	if($meta_id == 141)
	 	{	
		 	$p3_firstname = $result['__141'];
		 	$p3_last_name = $result['__143'];
		 	$p3_title = $result['__142'];
		 	$p3_phone = $result['__146'];
		 	$p3_phone_ext = $result['__147'];
		 	$p3_function = $result['__144'];
		 	$p3_email = $result['__145'];	

		 	$data = array("143"=>$d_last_name,"141"=>$d_firstname,"144"=>$d_function,"142"=>$d_title,"145"=>$d_email,"146"=>$d_phone,"147"=>$d_phone_ext);

		 	$data_d = array("183"=>$p3_last_name,"184"=>$p3_firstname,"89"=>$p3_function,"83"=>$p3_title,"85"=>$p3_email,"100"=>$p3_phone,"96"=>$p3_phone_ext);
	 	...
	 	if($meta_id == 148)
	 	{
		 	$p4_firstname = $result['__148'];
		 	$p4_last_name = $result['__150'];
		 	$p4_title = $result['__149'];
		 	$p4_phone = $result['__152'];
		 	$p4_phone_ext = $result['__246'];
		 	$p4_function = $result['__247'];
		 	$p4_email = $result['__151'];	

		 	$data = array("150"=>$d_last_name,"148"=>$d_firstname,"149"=>$d_title,"151"=>$d_email,"152"=>$d_phone,"246"=>$d_phone_ext,"247"=>$d_function);

		 	$data_d = array("183"=>$p4_last_name,"184"=>$p4_firstname,"89"=>$p4_function,"83"=>$p4_title,"85"=>$p4_email,"100"=>$p4_phone,"96"=>$p4_phone_ext);
	...
	public function qualify(){
		...
		$res=$this->App_model->getData('qualify_lead','*',['record_id'=>$get['id']]);

		if(empty($res)){

			$data=array('record_id'=>$get['id'],
						'file'=>$get['file'],
						'qualify_status'=>1,
						'qualified_by'=>$this->getUserId(),
						'created_time'=>time(),
						'modified_time'=>time()
						);

			$this->App_model->rowInsert('qualify_lead',$data);
			...
			$this->App_model->rowUpdate('qualify_lead',$data,['record_id'=>$get['id']]);
	    }


		$doc_data=array('record_id'=>$get['id'],
					'link'=>'http://anbruch.com/wp-content/themes/anbruch/lead-pdf/'.$get['file'],
					'name'=>$get['form_name'],
					'status'=>2,
		...
		$this->App_model->rowInsert('anb_crm_attachment',$doc_data);

		$this->db->flush_cache();

		$this->LogsModel->addLogs('Qualify', 'Lead is qualified' ,$get['id'],'Leads',$_SESSION['user_id'],'Qualify'); 

	   	redirect(base_url().'modules/viewRecord?cm=Leads&id='.$get['id'].'&record_status=3');

	}

	public function not_qualify_lead(){
		...
		$this->LogsModel->addLogs('Qualify', 'Lead is not qualified' ,$get['id'],'Leads',$_SESSION['user_id'],'Qualify');
       	
		 redirect(base_url().'modules/viewRecord?cm=Leads&id='.$get['id'].'&record_status=3');
	}

	function getRecentActivityDashboard(){
        ...
        $col_sort = array("id","title",'description','module_name','stage','activity_user','date');
       ...
       if (isset($_GET['iDisplayStart']) && $_GET['iDisplayLength'] != '-1') {
            $str_point = intval($_GET['iDisplayStart']);
            $lenght = intval($_GET['iDisplayLength']);
            $records = $this->db->from('anb_activities_log')
            		->select('*')
                    ->limit($lenght, $str_point)
                    ->get();
        } else {
	    ...
	    foreach ($res as $key => $value) {
	      	...
	      	$output['aaData'][] = array('','<a href="'. base_url() .'/modules/viewRecord/?cm='.$value['module_name'].'&id='. $value["record_id"] .'&record_status=3">'. $value["title"] .'</a>',
				  $value['description'],
			      $value['module_name'],
			      $value['stage'],
			      $value['activity_user'],		
			      date('m/d/y H:i',strtotime($value['date'])),
				);	
			}
     	}
...
function removeDuplicateString($dep) {
...
function convertToLead(){
	...
	$booking_data=$this->App_model->getData('anb_crm_web_booking','*',['booking_id'=>$post['web_id']]);
    ...
    $name=preg_split("/ /",$booking_data[0]['name']);
  
    $array['__1'] = $name[0];
    $array['__2'] = $name[1].' '.$name[2];
    
    $array['__3'] = $booking_data[0]['form_name'];
    $array['__4'] = $booking_data[0]['email'];	
    $array['__5'] = '';
    $array['__6'] = $post['web_lead_status'];
    $array['__7'] = ''; 
    $array['__8'] ='';
    $array['__9'] = $post['web_stype'];
    $array['__10'] = ''; 
    $array['__11'] = ''; 
    $array['__12'] = ''; 
    $array['__13'] = 0;
    ...
    $array['__203'] = '';
    $array['__204'] = '';
    $array['__21'] = $post['web_closer'];
    $array['__22'] =$post['web_opener']; 
    $array['__24'] = '';
    $array['__26'] = 'None';
    $array['__27'] = 'NONE';
    $array['__28'] = 'None';
    $array['__30'] = 'NONE';
    $array['__31'] = $booking_data[0]['company_name'];
    $array['__32'] = $booking_data[0]['phone'];
    $array['__33'] = '';
    $array['__34'] = '';
    $array['__35'] = $booking_data[0]['city'];
    $array['__36'] = '';
    $array['__37'] = $booking_data[0]['country'];
    $array['__38'] = '';
    $array['__39'] = '';
    $array['__40'] = $booking_data[0]['state'];
    $array['__41'] =''; 
    $array['__43'] =''; 
    $array['__44'] =''; 
    $array['__45'] =''; 
   ...
   if($res['status']){
		...
		$this->LogsModel->addLogs('Leads', 'Web booking Converted',$res['id'],'Leads',$this->getUserId(),'Prospect');
		
		$this->App_model->rowUpdate('anb_crm_web_booking',['lead_status'=>1],['booking_id'=>$post['web_id']]);

		$this->App_model->rowUpdate('anb_crm_bookings',['record_id'=>$res['id']],['id'=>$post['web_id']]);
 ...
 function delete_web_booking(){
 	$id=$this->input->post('id');
	$this->App_model->rowsDelete('anb_crm_bookings',['id'=>$id]);
	$this->db->flush_cache();
	$this->App_model->rowsDelete('anb_crm_web_booking',['booking_id'=>$id]);
	echo 'success';
}	
