	...
	<?php  $size = sizeof($surveyQ1)-1;	
		...
		for($i= 0 ; $i<sizeof($surveyQ1);$i++){
			...
			<input name="sub_ques_<?php echo $questionNo;?>" value="<?php echo $surveyQ1[$i]['sub_question']?>" type="hidden"/>
				...
				<button class="btn btn-success btn-xs btnSubmit1" type="submit" style="float:right;">Submit</button>
			...
			<input name="sub_ques_<?php echo $questionNo;?>" value="<?php echo $surveyQ1[$i]['sub_question']?>" type="hidden"/>
				...
				<?php } if($questionNo == 3){ ?>
					<button class="btn btn-success btn-xs" id="submit" type="submit" style="float:right; margin-left:10px !important;">Submit</button>
				<?php }?>	
...
$(document).ready(function(){
   $("input[type='radio']").click(function(){
		...
		if(elementName == 'input' && value == 'No'){
    			$(this).parent().parent().siblings('.btnNext').css('display', 'none');
    			$(this).parent().parent().siblings('.btnSubmit').css('display', 'block');	
    		}
    		else{
    			$(this).parent().parent().siblings('.btnNext').css('display', 'block');
    			$(this).parent().parent().siblings('.btnSubmit').css('display', 'none');	
    		}
    	});
		$('.btnNext').on('click',function(){
			var classStatus = $(this).parent().children().children().siblings().children().hasClass('others');
			var id = 1;
			if(classStatus){
				var hasClassDisabled = $(this).parent().children().children().siblings().children('.others').next().next("input[type='text']");
