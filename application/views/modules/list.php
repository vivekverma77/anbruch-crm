							...
							<div id="search_blk" style="display:none;position:absolute;z-index:100;background:#FFFFFF;width:1000px;height:auto; margin-top:10px; background:#f2f4f8;border:1px solid #DDDDDD;right:0px;padding:10px; -webkit-box-shadow: 0 2px 6px rgba(0,0,0,0.1); box-shadow:0 2px 6px rgba(0,0,0,0.1);">
								...
								<form name="frmSearch" id="frmSearch" method="get">
									...
									<p align="center" style="margin-top:12px;">
										<input type="submit" id="apply_filter" value="Apply Filters" class="btn btn-success btn-md" style="color:#fff !important;" />
						...
						<div class="adv-table">
							...
							<div class="row" style="margin-top:20px; padding:0 15px;">
								...
								<div class="col-lg-6">
									...
									<?php if ($current_module == "Leads") { ?>
									<button type="button" class="btn btn-yellow-o" title="Delete From Favorite List"data-id="Leads" id="remove_favorite_list_btn" <?php echo !empty($selected_fav_list) ? 'style="height: 32px;"':'style="display:none;"'; ?> > <i class="icon-crm-delete"></i>
									</button>

									<?php } ?>
									<?php if ($current_module == "Clients") { ?>

										<button type="button" class="btn btn-blue-o" title="Delete From Favorite List" data-id="Clients" id="remove_favorite_list_btn" <?php echo !empty($selected_fav_list) ? 'style="height: 32px;"':'style="display:none;"'; ?> > <i class="icon-crm-delete"></i>
										</button>
									<?php } ?>
									<?php if ($current_module == "Contracts") { ?>
										<button type="button" class="btn btn-green-o" title="Delete From Favorite List" data-id="Contracts" id="remove_favorite_list_btn" <?php echo !empty($selected_fav_list) ? 'style="height: 32px;"':'style="display:none;"'; ?> > <i class="icon-crm-delete"></i>
										</button>
		...
		<tr style="font-weight:bold;">
			...
			foreach ($record_data as $key => $record)
			{
				foreach ($record['meta'] as $metaFieldName => $metaValue)
				{
					...
					$class='all';
					if($metaFieldName=='note'){
						$class='none';
					}
				?>
				<td nowrap="nowrap" class="<?php echo $class ?>">
		...
		<tr>
			...
			foreach ($record['meta'] as $metaFieldName => $metaValue)
			{
				...
				<?php
                 ...
                 if($record['static']['reminder_count'] > 0){
                      $class='';
                      $classCount='';  
                 }
				...
				<td>
					 ...
					 <a class="reminder-btn reminders <?php echo $class; ?>" company_name="<?php echo $column_value;?>" record_id="<?php echo $key ?>" id="reminders" href="javascript:void(0);" title="Reminders" ><i class="fa fa-clock-o"></i><span class="reminder-count <?php echo $classCount; ?>" id="reminder-count-<?php echo $key ?>"><?php echo $record['static']['reminder_count']; ?></span></a>				
				...
				<?php } ?>
				<?php
					if ($current_module=='Contracts' && $metaFieldName == 'Contract Client')
				{?>
				<td>
				...
				<?php
					if ($current_module=='Leads' && $metaFieldName == 'Company Name')
					{
						echo '<a class="list_link" href="' . base_url() . 'modules/viewRecord/?cm=' . $current_module  . "&id=$key&record_status=" . $record['static']['record_status'] . '&alphabet='.$alphabet. '&sper_page='.$sper_page. '&spage='.$spage.'&fl_id='.$fl_id.'">';
					}
					...
					if ($current_module=='Clients' && $metaFieldName == 'Client Name')
					{
						echo '<a class="list_link_clients" href="' . base_url() . 'modules/viewRecord/?cm=' . $current_module  . "&id=$key&record_status=" . $record['static']['record_status'] . '&alphabet='.$alphabet.'&spage='.$spage.'&fl_id='.$fl_id.'">';
					}
					...
					{
						echo '<a class="list_link_contracts" href="' . base_url() . 'modules/viewRecord/?cm=' . $current_module  . "&id=$key&record_status=" . $record['static']['record_status'] . '&alphabet='.$alphabet.'&spage='.$spage.'&fl_id='.$fl_id.'">';
					}
...
<!-- for existing -->					
					<div class="modal-body existing_list">
							<div class="form-group col-lg-6" style="min-height: 57px !important;">
									<!-- <label style="margin-left: 6px;">List *</label>
									<br> -->
						<select id="own11"  name="own11" class="populate" style="max-width: 50%;margin-top: -2px;margin-left: 5px;min-width: 15%;">
							...
							<div class="clearfix">
								<button class="btn btn-success for_new_create" style="float: right;">Create New</button>
							</div>
					</div>
					<div class="modal-footer existing_list_footer">
							<button data-dismiss="modal" class="btn btn-default" type="button">Ignore</button>
							<button class="btn btn-success" id="add_fav_new" type="button">Add</button>
					</div>
<!-- for existing -->					


<!-- 	for new -->		
					<div class="modal-body new_list" style="display: none;">
							<div class="form-group col-lg-6" style="min-height: 57px !important;">
									<label>Title *</label>
									<input type="text" id="title" class="form-control" placeholder="title" />
							</div>
							<div class="clearfix">
								<button class="btn btn-success for_exist_list" style="float: right;">Exist List</button>
							</div>
					</div>
					<div class="modal-footer new_list_footer" style="display: none;">
							<button data-dismiss="modal" class="btn btn-default" type="button">Ignore</button>
...
<!-- Reminder model end-->
   <?php
	        ...
	        if(isset($_GET['num_records_per_page']) && !empty($_GET['num_records_per_page'])){ $Lead_select = $_GET['num_records_per_page']; } else { $Lead_select = "";}

	        if(isset($_GET['own']) && !empty($_GET['own'])){ $Lead_record_of = $_GET['own']; } else { $Lead_record_of = "";}

	        if(isset($_GET['keyword']) && !empty($_GET['keyword'])){ $keyword = $_GET['keyword']; } else { $keyword = "";} 
...
<script type="text/javascript">
	...
	$(document).ready(function(){
	  ...
	  //Remind start
		...
		var current_module = "<?php echo $current_module;?>";

		$(document).on("change","#rStat,#own1",function(){
			if(current_module == "Leads")
			  { 	
				localStorage.setItem("Lead_select", "");
				localStorage.setItem("Lead_record_of", "");
				localStorage.setItem("Lead_search", "");
			  }	
			if(current_module == "Clients")
			  { 	
				localStorage.setItem("Client_select", "");
				localStorage.setItem("Client_record_of", "");
				localStorage.setItem("Client_search", "");
			  }	
			if(current_module == "Contracts")
			  { 	
				localStorage.setItem("Contract_select", "");
				localStorage.setItem("Contract_record_of", "");
				localStorage.setItem("Contract_search", "");
			  }	    
		});
		$(document).on("click",".alphabet-links,#apply_filter",function(){
			if(current_module == "Leads")
			  { 	
				localStorage.setItem("Lead_select", "");
				localStorage.setItem("Lead_record_of", "");
		...
		$(document).on("click","#num_records_per_page",function(){
				var value = $(this).val();
				if(current_module == "Leads")
				{localStorage.setItem("Lead_select", value);}
				if(current_module == "Clients")
				{localStorage.setItem("Client_select", value);}
			    if(current_module == "Contracts")
				{localStorage.setItem("Contract_select", value);}
		});
		$(document).on("click","#own",function(){
				var value = $(this).val();
				if(current_module == "Leads")
				{localStorage.setItem("Lead_record_of", value);}
				if(current_module == "Clients")
				{localStorage.setItem("Client_record_of", value);}
			    if(current_module == "Contracts")
				{localStorage.setItem("Contract_record_of", value);}
		});
		$(document).on("keyup","#keyword",function(){
				var value = $(this).val();
				if(current_module == "Leads")
				{localStorage.setItem("Lead_search", value);}
				if(current_module == "Clients")
				{localStorage.setItem("Client_search", value);}
			    if(current_module == "Contracts")
				{localStorage.setItem("Contract_search", value);}
		});
		if(current_module == "Leads")
		{
			if(localStorage.getItem("Lead_select"))
			{
				var Lead_select = "<?php echo $Lead_select; ?>";
				if(Lead_select == "" && localStorage.getItem("Lead_select") != 25)
				{
				 	var value = localStorage.getItem("Lead_select");
				 	$("#num_records_per_page").val(value).trigger('change');
				}	
			}
			if(localStorage.getItem("Lead_record_of"))
			{
				var Lead_record_of = "<?php echo $Lead_record_of; ?>";
				if(Lead_record_of == "")
				{
				 var value = localStorage.getItem("Lead_record_of");
				 $("#own").val(value).trigger('change');
				}	
			}
			if(localStorage.getItem("Lead_search"))
			{
				var keyword = "<?php echo $keyword; ?>";
				if(keyword == "")
				{
				 var value = localStorage.getItem("Lead_search");
				 $("#keyword").val(value);
		...
		if(current_module == "Clients")
		{
			if(localStorage.getItem("Client_select"))
			{
				var Client_select = "<?php echo $Lead_select; ?>";
				if(Client_select == "" && localStorage.getItem("Client_select") != 25)
				{
				 var value = localStorage.getItem("Client_select");
				 $("#num_records_per_page").val(value).trigger('change');
				}	
			}
			if(localStorage.getItem("Client_record_of"))
			{
				var Client_record_of = "<?php echo $Lead_record_of; ?>";
				if(Client_record_of == "")
				{
				 var value = localStorage.getItem("Client_record_of");
				 $("#own").val(value).trigger('change');
				}	
			}
			if(localStorage.getItem("Client_search"))
			{
				var keyword = "<?php echo $keyword; ?>";
				if(keyword == "")
				{
				 var value = localStorage.getItem("Client_search");
				 $("#keyword").val(value);
				 $(".keyword_search").trigger('click');
				}	
			}
		...
		if(current_module == "Contracts")
		{
			if(localStorage.getItem("Contract_select"))
			{
				var Contract_select = "<?php echo $Lead_select; ?>";
				if(Contract_select == "" && localStorage.getItem("Contract_select") != 25)
				{
				 var value = localStorage.getItem("Contract_select");
				 $("#num_records_per_page").val(value).trigger('change');
				}	
			}
			if(localStorage.getItem("Contract_record_of"))
			{
				var Contract_record_of = "<?php echo $Lead_record_of; ?>";
				if(Contract_record_of == "")
				{
				 var value = localStorage.getItem("Contract_record_of");
				 $("#own").val(value).trigger('change');
				}	
			}
			if(localStorage.getItem("Contract_search"))
			{
				var keyword = "<?php echo $keyword; ?>";
				if(keyword == "")
				{
				 var value = localStorage.getItem("Contract_search");
				 $("#keyword").val(value);
				 $(".keyword_search").trigger('click');
				}	
			}
	  ...
	  //Remind end
       ...
       $(document).on('click','#addFavoriteListModal .for_new_create',function(){
            $(".new_list").css('display','block');
            $(".new_list_footer").css('display','block');

            $(".existing_list").css('display','none');
            $(".existing_list_footer").css('display','none');
      }); 

      $(document).on('click','#addFavoriteListModal .for_exist_list',function(){
            $(".new_list").css('display','none');
            $(".new_list_footer").css('display','none');

            $(".existing_list").css('display','block');
            $(".existing_list_footer").css('display','block');           
      });
    ...
    $(document).ready(function() {
    	...
    	$("#remove_favorite_list_btn").click(function(){
    		var fav_id = $("#own11").children("option:selected").val();	
			var uids = '';
			...
			else
			{
				...
				$.ajax({
					type:'post',
					url: base_url + 'modules/removeFavoriteList',
					dataType:'json',
