...
class ModuleModel extends BaseModel
{
 ...
 return $ret;
	...
	function loadQualified($user_id, $record_id)    
	{
		...
		$res = $this->db->get('qualify_answers qn');
 ...
 */
		...
		if($owner){ $this->db->where("r.assigned_officer_id", $owner); }
		if(isset($search_filter) && count($search_filter))
		{
			foreach($search_filter as $k => $v)
			{
	...
	*/	
		...
		if($page_id == 1){
				$sort_order = 'DESC';
				$sort_id = 'ID';
			$last_sql = str_replace('DISTINCT `r`.`id`', "r.*, $extraSelect$additionalFieldName", $this->db->last_query())." ORDER BY `$sort_id` $sort_order LIMIT $start_rec_no, ". $limits_per_page;
		}
		else{
		...
		foreach ($res->result_array() as $record) 
		{
            ...
            if($module_name=='Leads'){
              ...
              unset($ret[$record["id"]]['meta']['Country']);
              unset($ret[$record["id"]]['meta']['Estimated Sales']);
              unset($ret[$record["id"]]['meta']['Reported Sales']);
              unset($ret[$record["id"]]['meta']['First Name']);
              unset($ret[$record["id"]]['meta']['Last Name']);
              unset($ret[$record["id"]]['meta']['Fax']);
              unset($ret[$record["id"]]['meta']['Title']);
              unset($ret[$record["id"]]['meta']['Lead Status']);
              unset($ret[$record["id"]]['meta']['Email']);
              unset($ret[$record["id"]]['meta']['Opener']);
              unset($ret[$record["id"]]['meta']['Closer']);
              unset($ret[$record["id"]]['meta']['Lead Source']);
              unset($ret[$record["id"]]['meta']['Lead Referred Partner']);
		...
		/*if($module_name == "Leads"){
			...
			foreach ($ret as $key => $value) {	
				...
				$time = $modifiedtime->result_array();
					if(empty($time)){
					$ret[$key]["meta"]["Modified Time"] = 'Not modified';	
					}else{
						foreach ($time as $mtime) {
							//print_r($mtime);
						$ret[$key]["meta"]["Modified Time"] = !empty($mtime['modified_time']) ? date('m/d/Y H:i',$mtime['modified_time']) : 'Not modified';
					}
				}
			}
		}*/
...
/*        $this->db->where("r.record_status", $RECORD_STATUS['CYCLE_RUNNING']);*/
	...
	function convertRecord($userId, $record_Id=null, $expected_technical_start_date)
	{
		...
		if ($recordCreationSuccessful === true)
		{
			echo $client_id = $this->db->insert_id();
			$contract_ids = array();
	...
	function changeOwner($userId, $massOwner = false, $ids = null, $newOwner = null)
	{
		...
		foreach ($idList as $key => $id){
			...
			if($module == 'Leads'){
				$this->db->update('anb_crm_record_meta_value', array('value'=>$owner), array('record_meta_id' => 21, 'record_id'=>$id));
				$this->db->update('anb_crm_record_meta_value', array('value'=>'ASSIGNED TO CLOSER'), array('record_meta_id' => 6, 'record_id'=>$id));	
			}else{
				$this->db->update('anb_crm_record_meta_value', array('value'=>$owner), array('record_meta_id' => 105, 'record_id'=>$id));
			}
		}
	...
	public function get_latest_leads($limit,$offset,$order_by){
	    
	    $this->db->select('*');
        $this->db->from("(SELECT * FROM `anb_activities_log` WHERE `title` = 'Leads' ORDER by id  DESC ) acl"); 
    ...
    public function get_leads_data($record_id,$module_id,$search){
    	 
    	 $this->db->select('mv.value,m.name,r.record_status');
    	 $this->db->from('anb_crm_record_meta_value mv');
    	 $this->db->join('anb_crm_record r','r.id=mv.record_id');
    	 $this->db->join('anb_crm_record_meta m','m.id=mv.record_meta_id');
    	 if(!empty($search)){
    	 	$this->db->like('mv.value',$search);
    	 }
    ...
    public function total_last_updated_lead_record(){
	...
	public function get_leads_by_user_id($limit,$offset,$order_by,$user_id,$col){
        ...
        if($col!='id'){
	        ...
	        $this->db->join('anb_crm_record_meta_value mv','mv.record_id=r.id');
	        $this->db->join("(select id from anb_crm_record_meta acrm where module_id =1 and name= '".$col."') sub1 ",'sub1.id=mv.record_id'); 
            $this->db->order_by('mv.value',$order_by);
        }
	    $this->db->where('r.module_id',1);
	    $this->db->where('r.record_status',3);
        if(!empty($user_id))
          $this->db->where('r.assigned_officer_id',$user_id); 

    	$this->db->limit($limit,$offset);
    	//$this->db->order_by('r.id',$order_by);
    	$query=$this->db->get('anb_crm_record r');
    	echo $this->db->last_query();die();
        return $query->result_array();
    }

    public function total_leads_by_user_id($id){
    	$this->db->from('anb_crm_record');
    	$this->db->where('module_id',1);
        $this->db->where('record_status',3);
        if(!empty($user_id))
          $this->db->where('assigned_officer_id',$user_id); 

        return $this->db->count_all_results();
    } 

	function loadRecordList_beta($recordStatus = null, $owner = null, $search_filter = null, $npage_no = 1, $limit = 25, $sort_column = '', $sort_order = '', $keyword = '', $module_name = '' , $from_date ='', $page_id ='',$alphabet='')
	{
 ...
 */
	...
	*/	if($page_id == 1){  
			...
			if($module_name == 'Leads')
			{
				if(!empty($alphabet))
				{
					$last_sql = str_replace('DISTINCT `r`.`id`', "r.*, $extraSelect$additionalFieldName", $this->db->last_query())."AND ad31.value like '".$alphabet."%' ORDER BY `$sort_id` $sort_order LIMIT $start_rec_no, ". $limits_per_page;
				}
				else
				{
					$last_sql = str_replace('DISTINCT `r`.`id`', "r.*, $extraSelect$additionalFieldName", $this->db->last_query())."ORDER BY `$sort_id` $sort_order LIMIT $start_rec_no, ". $limits_per_page;	
